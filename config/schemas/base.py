...
class JiraConfig(BaseModel):
    ...
    fields: str = Field(
        default="summary,description,issuetype,status,priority,assignee,reporter,created,updated,labels,components,fixVersions,versions,comment,attachment,worklog,customfield_*",
        description="Jira fields to retrieve"
    )
    scriptrunner_enabled: bool = Field(default=False, description="Enable ScriptRunner integration")
    scriptrunner_endpoint: Optional[str] = Field(default=None, description="ScriptRunner endpoint")
    ...
    def validate_scriptrunner_endpoint(cls, v: Optional[str], values: dict) -> Optional[str]:
        """Validate ScriptRunner endpoint if enabled."""
        if values.get('scriptrunner_enabled') and not v:
            raise ValueError('ScriptRunner endpoint is required when ScriptRunner is enabled')
        return v
...
class OpenProjectConfig(BaseModel):
    ...
    api_key: Optional[str] = Field(default=None, description="Alternative API key")
...
class MigrationConfig(BaseModel):
    ...
    batch_size: int = Field(default=100, ge=1, le=1000, description="Migration batch size")
    ...
    component_order: List[str] = Field(
        default=["users", "custom_fields", "projects", "versions", "issues", "relations", "worklogs", "attachments", "comments"],
        description="Migration component order"
    )
    mapping_file: Path = Field(default=Path("data/id_mapping.json"), description="ID mapping file")
    attachment_path: Path = Field(default=Path("data/attachments"), description="Attachment storage path")
    
    @validator('data_dir', 'backup_dir', 'results_dir', 'mapping_file', 'attachment_path')
    ...
    def validate_component_order(cls, v: List[str]) -> List[str]:
        ...
        valid_components = {
            "users", "custom_fields", "projects", "versions", 
            "issues", "relations", "worklogs", "attachments", "comments"
        }
        
        for component in v:
            if component not in valid_components:
                raise ValueError(f'Invalid component: {component}. Valid components: {", ".join(valid_components)}')
