...
class TimestampCorrectionScript:
    ...
    def _process_batch(self, work_packages: list[dict[str, Any]]) -> None:
        ...
        for wp in work_packages:
            try:
                ...
                self.logger.error("Failed to process work package %s: %s", 
                                wp.get("id", "unknown"), e)
                self.stats["errors"] += 1
    ...
    def _process_work_package(self, work_package: dict[str, Any]) -> None:
        ...
        for field_id, field_data in custom_fields.items():
            if isinstance(field_data, dict) and "value" in field_data:
                ...
                if timestamp_str and self._is_timestamp_field(field_data) and \
                   self._needs_timezone_correction(timestamp_str):
                    corrections_needed.append({
    ...
    def _needs_timezone_correction(self, timestamp_str: str) -> bool:
        ...
        try:
            ...
            elif '+' in timestamp_str or timestamp_str.endswith(tuple(f'{h:02d}:00' for h in range(24))):
    ...
    def _log_corrections(self, wp_id: str, corrections: list[dict[str, Any]]) -> None:
        ...
        """
        self.logger.info("Work package %s - %d timestamp corrections needed:", 
                        wp_id, len(corrections))
    ...
    def _apply_corrections(self, wp_id: str, corrections: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            # Apply the update
            response = self.op_client.patch(f"/api/v3/work_packages/{wp_id}", data=update_data)
            
            if response:
                self.logger.info("Successfully corrected %d timestamps for work package %s", 
                               len(corrections), wp_id)
            else:
                self.logger.error("Failed to update work package %s", wp_id)
                self.stats["errors"] += 1
                
        except Exception as e:
            self.logger.error("Failed to apply corrections to work package %s: %s", wp_id, e)
            self.stats["errors"] += 1
    ...
    def _print_final_report(self) -> None:
        ...
        self.logger.info("Total work packages processed: %d", self.stats["total_packages"])
        self.logger.info("Work packages with timestamps: %d", self.stats["packages_with_timestamps"])
