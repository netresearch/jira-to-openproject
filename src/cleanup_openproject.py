...
class OpenProjectCleaner:
    ...
    def bulk_delete_work_packages(self) -> int:
        ...
        """
        logger.info("Starting bulk work package deletion via Rails console...", extra={"markup": True})

        if not self._ensure_rails_client():
            logger.error("Cannot bulk delete work packages: Rails client not available or connected.")
            return 0

        if self.dry_run:
            logger.info("DRY RUN: Would delete all work packages via Rails console", extra={"markup": True})
            # Get count for reporting
            result = self.rails_client.execute("WorkPackage.count")
            if result['status'] == 'success':
                count = int(result.get('output', '0').strip())
                logger.info(f"DRY RUN: Would delete {count} work packages", extra={"markup": True})
                return count
            return 0
        ...
        if delete_result['status'] == 'success':
            logger.success(f"Successfully deleted all {wp_count} work packages", extra={"markup": True})
            return wp_count
        else:
            error_msg = delete_result.get('error', 'Unknown execution error')
            logger.error(f"Error executing Rails command for bulk work package deletion: {error_msg}", extra={"markup": True})
            return 0
    ...
    def bulk_delete_projects(self) -> int:
        ...
        """
        logger.info("Starting bulk project deletion via Rails console...", extra={"markup": True})

        if not self._ensure_rails_client():
            logger.error("Cannot bulk delete projects: Rails client not available or connected.")
            return 0

        if self.dry_run:
            logger.info("DRY RUN: Would delete all projects via Rails console", extra={"markup": True})
            # Get count for reporting
            result = self.rails_client.execute("Project.count")
            if result['status'] == 'success':
                count = int(result.get('output', '0').strip())
                logger.info(f"DRY RUN: Would delete {count} projects", extra={"markup": True})
                return count
            return 0
        ...
        if delete_result['status'] == 'success':
            logger.success(f"Successfully deleted all {project_count} projects", extra={"markup": True})
            return project_count
        else:
            error_msg = delete_result.get('error', 'Unknown execution error')
            logger.error(f"Error executing Rails command for bulk project deletion: {error_msg}", extra={"markup": True})
            return 0
    ...
    def cleanup_issue_types(self) -> int:
        ...
        if not self._ensure_rails_client():
            logger.error("Cannot clean up issue types: Rails client not available or connected.")
            return 0
        ...
        try:
            ...
            if not type_ids:
                logger.info("No non-default issue types found to delete", extra={"markup": True})
                return 0

            logger.info(f"Found {len(type_ids)} non-default issue types to delete", extra={"markup": True})

            if self.dry_run:
                logger.info("DRY RUN: Would delete non-default issue types", extra={"markup": True})
                for type_id in type_ids:
                    logger.info(f"DRY RUN: Would delete issue type with ID: {type_id}", extra={"markup": True})
                return len(type_ids)

            # Use bulk deletion for efficiency if there are many types
            if len(type_ids) > 20:
                logger.info(f"Using bulk deletion for {len(type_ids)} issue types", extra={"markup": True})
                delete_cmd = """
                ...
                if delete_result['status'] == 'success':
                    logger.success(f"Successfully deleted all {len(type_ids)} non-default issue types", extra={"markup": True})
                    return len(type_ids)
                else:
                    error_msg = delete_result.get('error', 'Unknown execution error')
                    logger.error(f"Error executing bulk deletion of issue types: {error_msg}", extra={"markup": True})
                    return 0

            # For smaller numbers, delete individually to track progress
            deleted_count = 0
            with ProgressTracker("Deleting issue types", len(type_ids), "Recent Issue Types") as tracker:
                for type_id in type_ids:
                    ...
                    try:
                        ...
                        if success:
                            ...
                            error_msg = delete_result.get('error', 'Unknown execution error')
                            logger.error(f"Error deleting issue type {type_id}: {error_msg}", extra={"markup": True})
                            tracker.add_log_item(f"Failed: Type ID {type_id}")
                    except (ValueError, TypeError) as e:
                        logger.error(f"Invalid type ID format: {type_id} - {str(e)}", extra={"markup": True})
                        tracker.add_log_item(f"Failed (invalid ID format): {type_id}")

                    tracker.increment(1, f"Deleting issue types ({deleted_count}/{len(type_ids)})")

            logger.success(f"Successfully deleted {deleted_count} out of {len(type_ids)} issue types", extra={"markup": True})
            return deleted_count

        except json.JSONDecodeError:
            logger.error("Failed to parse issue types from Rails output", extra={"markup": True})
            return 0
    ...
    def cleanup_issue_statuses(self) -> int:
        ...
        if not self._ensure_rails_client():
            logger.error("Cannot clean up issue statuses: Rails client not available or connected.")
            return 0
        ...
        try:
            ...
            if not status_ids:
                logger.info("No non-default issue statuses found to delete", extra={"markup": True})
                return 0

            logger.info(f"Found {len(status_ids)} non-default issue statuses to delete", extra={"markup": True})

            if self.dry_run:
                logger.info("DRY RUN: Would delete non-default issue statuses", extra={"markup": True})
                for status_id in status_ids:
                    logger.info(f"DRY RUN: Would delete issue status with ID: {status_id}", extra={"markup": True})
                return len(status_ids)

            # Use bulk deletion for efficiency if there are many statuses
            if len(status_ids) > 20:
                logger.info(f"Using bulk deletion for {len(status_ids)} issue statuses", extra={"markup": True})
                delete_cmd = """
                ...
                if delete_result['status'] == 'success':
                    logger.success(f"Successfully deleted all {len(status_ids)} non-default issue statuses", extra={"markup": True})
                    return len(status_ids)
                else:
                    error_msg = delete_result.get('error', 'Unknown execution error')
                    logger.error(f"Error executing bulk deletion of issue statuses: {error_msg}", extra={"markup": True})
                    return 0

            # For smaller numbers, delete individually to track progress
            deleted_count = 0
            with ProgressTracker("Deleting issue statuses", len(status_ids), "Recent Issue Statuses") as tracker:
                for status_id in status_ids:
                    ...
                    try:
                        ...
                        if success:
                            ...
                            error_msg = delete_result.get('error', 'Unknown execution error')
                            logger.error(f"Error deleting issue status {status_id}: {error_msg}", extra={"markup": True})
                            tracker.add_log_item(f"Failed: Status ID {status_id}")
                    except (ValueError, TypeError) as e:
                        logger.error(f"Invalid status ID format: {status_id} - {str(e)}", extra={"markup": True})
                        tracker.add_log_item(f"Failed (invalid ID format): {status_id}")

                    tracker.increment(1, f"Deleting issue statuses ({deleted_count}/{len(status_ids)})")

            logger.success(f"Successfully deleted {deleted_count} out of {len(status_ids)} issue statuses", extra={"markup": True})
            return deleted_count

        except json.JSONDecodeError:
            logger.error("Failed to parse issue statuses from Rails output", extra={"markup": True})
            return 0
    ...
    def cleanup_issue_link_types(self) -> int:
        ...
        if not self._ensure_rails_client():
            logger.error("Cannot clean up issue link types: Rails client not available or connected.")
            return 0
        ...
        if check_result['status'] != 'success' or check_result.get('output', '').strip().lower() != 'true':
            logger.warning("TypedRelation model not found in this OpenProject instance. Skipping relation type cleanup.", extra={"markup": True})
            return 0
        ...
        try:
            ...
            if not custom_link_types:
                logger.info("No custom issue link types found to delete", extra={"markup": True})
                return 0

            logger.info(f"Found {len(custom_link_types)} custom issue link types to delete", extra={"markup": True})

            if self.dry_run:
                logger.info("DRY RUN: Would delete custom issue link types", extra={"markup": True})
                for link_type in custom_link_types:
                    ...
                    logger.info(f"DRY RUN: Would delete issue link type: {link_name}/{reverse_name} (ID: {link_id})", extra={"markup": True})
                return len(custom_link_types)

            deleted_count = 0
            with ProgressTracker("Deleting issue link types", len(custom_link_types), "Recent Issue Link Types") as tracker:
                for link_type in custom_link_types:
                    link_id = link_type.get('id')
                    ...
                    try:
                        ...
                        if success:
                            deleted_count += 1
                            tracker.add_log_item(f"Deleted: {link_name}/{reverse_name} (ID: {link_id})")
                        else:
                            error_msg = delete_result.get('error', 'Unknown execution error')
                            logger.error(f"Error deleting issue link type {link_id}: {error_msg}", extra={"markup": True})
                            tracker.add_log_item(f"Failed: {link_name}/{reverse_name} (ID: {link_id})")
                    except (ValueError, TypeError) as e:
                        logger.error(f"Invalid link type ID format: {link_id} - {str(e)}", extra={"markup": True})
                        tracker.add_log_item(f"Failed (invalid ID format): {link_id}")

                    tracker.increment(1, f"Deleting issue link types ({deleted_count}/{len(custom_link_types)})")

            logger.success(f"Successfully deleted {deleted_count} out of {len(custom_link_types)} issue link types", extra={"markup": True})
            return deleted_count

        except json.JSONDecodeError:
            logger.error("Failed to parse issue link types from Rails output", extra={"markup": True})
            return 0
    ...
    def cleanup_custom_fields(self) -> int:
        ...
        """
        logger.info("Starting custom field cleanup via Rails console...", extra={"markup": True})

        if not self._ensure_rails_client():
            logger.error("Cannot bulk delete custom fields: Rails client not available or connected.")
            return 0

        if self.dry_run:
            logger.info("DRY RUN: Would delete custom fields via Rails console", extra={"markup": True})
            # Get count for reporting
            result = self.rails_client.execute("CustomField.count")
            if result['status'] == 'success':
                count = int(result.get('output', '0').strip())
                logger.info(f"DRY RUN: Would delete {count} custom fields", extra={"markup": True})
                return count
            return 0
        ...
        if delete_result['status'] == 'success':
            logger.success(f"Successfully deleted all {cf_count} custom fields", extra={"markup": True})
            return cf_count
        else:
            error_msg = delete_result.get('error', 'Unknown execution error')
            logger.error(f"Error executing Rails command for bulk custom field deletion: {error_msg}", extra={"markup": True})
            return 0
    ...
    def run_cleanup(self) -> Dict[str, int]:
        ...
        for entity_type in cleanup_order:
            if entity_type in self.entities:
                if entity_type == 'wp':
                    ...
                    results["issue_link_types_deleted"] = self.cleanup_issue_link_types()
        ...
        if 'it' in self.entities:
             logger.info(f"  Issue types deleted: {results['issue_types_deleted']}", extra={"markup": True})
        if 'is' in self.entities:
             logger.info(f"  Issue statuses deleted: {results['issue_statuses_deleted']}", extra={"markup": True})
        if 'il' in self.entities:
             logger.info(f"  Issue link types deleted: {results['issue_link_types_deleted']}", extra={"markup": True})
...
def main():
    ...
    default_entities = ['u', 'wp', 'p', 'cf', 'it', 'is', 'il'] # Internal representation
