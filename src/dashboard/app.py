...
class ConnectionManager:
    ...
    async def broadcast_progress(self, progress: MigrationProgress):
    ...
    async def broadcast_metrics(self, metrics: MigrationMetrics):
        ...
        try:
            if migration_state["is_running"] and migration_state["migration_id"]:
                ...
                if migration_state["error_recovery_system"]:
                    progress_data = await migration_state["error_recovery_system"].get_progress()
...
async def handle_control_command(command: Dict[str, Any], websocket: WebSocket):
...
async def start_migration_background(config: Dict[str, Any]):
...
async def stop_migration_background():
...
async def pause_migration_background():
...
async def resume_migration_background():
...
async def get_progress(migration_id: Optional[str] = None) -> JSONResponse:
...
@app.get("/api/metrics")
async def get_metrics(migration_id: Optional[str] = None) -> JSONResponse:
...
async def get_metrics_csv(migration_id: Optional[str] = None) -> JSONResponse:
    ...
    try:
        ...
        for key, value in metrics_data.items():
            if key != "migration_id" and key != "throughput_history":
                csv_content += f"{timestamp},{key},{value}\n"
        
        return JSONResponse(content={
            "csv_content": csv_content,
            "filename": f"migration_metrics_{migration_id or 'current'}_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.csv"
        })
...
async def start_migration(control: MigrationControl, background_tasks: BackgroundTasks) -> JSONResponse:
