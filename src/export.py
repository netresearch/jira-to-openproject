...
from src.config import get_path, logger, migration_config
from src.display import ProgressTracker
from src.migrations.work_package_migration import WorkPackageMigration
from src.mappings.mappings import Mappings


def import_project_work_packages(
    export_file: str,
    export_dir: str,
    op_client: Any,
    container_name: str,
    ...
    logger.notice(
        f"=== IMPORTING WORK PACKAGES FOR PROJECT {current_project_key} ===",
        extra={"markup": True},
    )
    ...
    try:
        ...
        logger.error(
            f"Error reading export file {export_file}: {str(e)}", extra={"markup": True}
        )
        return {
            "status": "error",
    ...
    if not work_packages_data:
        logger.warning(
            f"No work package data found in export file {export_file}",
            extra={"markup": True},
        )
        return {
            "status": "warning",
    ...
    logger.notice(
        f"Importing {len(work_packages_data)} work packages for project {current_project_key}",
        extra={"markup": True},
    )
    ...
    try:
        if op_server:
            ...
            logger.info(
                f"Copying file to {op_server} container {container_name}",
                extra={"markup": True},
            )
            # First copy to the server
            scp_cmd = ["scp", export_file, f"{op_server}:/tmp/"]
            logger.debug(
                f"Running command: {' '.join(scp_cmd)}", extra={"markup": True}
            )
            ...
            ssh_cmd = [
                ...
                "docker",
                "cp",
                f"/tmp/{os.path.basename(export_file)}",
                f"{container_name}:{container_temp_path}",
            ]
            logger.debug(
                f"Running command: {' '.join(ssh_cmd)}", extra={"markup": True}
            )
            subprocess.run(ssh_cmd, check=True)
        else:
            # Direct docker cp
            logger.info(
                f"Copying file to container {container_name}", extra={"markup": True}
            )
            docker_cp_cmd = [
                "docker",
                "cp",
            ...
            logger.debug(
                f"Running command: {' '.join(docker_cp_cmd)}", extra={"markup": True}
            )
        ...
        logger.error(
            f"Error copying file to Docker container: {str(e)}", extra={"markup": True}
        )
        return {
            "status": "error",
    ...
    logger.notice(
        f"Importing {len(work_packages_data)} work packages via Rails console",
        extra={"markup": True},
    )
    ...
    try:
        ...
        logger.success(
            f"Import completed: {result_data.get('created_count', 0)} work packages created, {result_data.get('error_count', 0)} errors",
            extra={"markup": True},
        )

        # Write detailed report to file
        report_file = os.path.join(
            export_dir, f"import_report_{current_project_key}.json"
        )
        with open(report_file, "w") as f:
            json.dump(result_data, f, indent=2)

        logger.info(
            f"Detailed import report saved to {report_file}", extra={"markup": True}
        )
...
def export_work_packages(
    ...
    op_rails_client: OpenProjectRailsClient | None,
    dry_run: bool = False,
    force: bool = False,
    ...
    try:
        ...
        work_package_migration = WorkPackageMigration(
            jira_client, op_client, op_rails_client, data_dir=config.get_path("data")
        )
        ...
        if not mappings.has_mapping("projects"):
            logger.error(
                "Project mapping not found. Run the project migration first.",
                extra={"markup": True},
            )
            return {
                "status": "error",
        ...
        logger.info(
            f"Found {len(project_mapping)} projects in mapping file",
            extra={"markup": True},
        )
        ...
        if project_keys:
            filtered_mapping = {
                k: v for k, v in project_mapping.items() if k in project_keys
            }
            if not filtered_mapping:
                logger.error(
                    f"None of the specified projects {project_keys} found in mapping",
                    extra={"markup": True},
                )
                return {
                    "status": "error",
            ...
            logger.info(
                f"Filtered to {len(project_mapping)} specified projects: {list(project_mapping.keys())}",
                extra={"markup": True},
            )
        ...
        for jira_project_key, op_project_id in project_mapping.items():
            ...
            if not op_project_id:
                logger.warning(
                    f"OpenProject ID missing for Jira project {jira_project_key}, skipping",
                    extra={"markup": True},
                )
            ...
            logger.notice(
                f"Processing project: {jira_project_key} → OpenProject ID {op_project_id}",
                extra={"markup": True},
            )
            ...
            try:
                ...
                export_file = os.path.join(
                    export_dir, f"work_packages_{jira_project_key}.json"
                )
                if os.path.exists(export_file) and not force:
                    logger.info(
                        f"Export file already exists for {jira_project_key}. Use --force to regenerate.",
                        extra={"markup": True},
                    )
                    ...
                    try:
                        with open(export_file) as f:
                            ...
                            logger.info(
                                f"Found {wp_count} work packages in existing export",
                                extra={"markup": True},
                            )
                            ...
                            progress.advance(
                                1,
                                f"Skipped {jira_project_key} (already exported: {wp_count} WPs)",
                            )
                            continue
                    except (json.JSONDecodeError, FileNotFoundError) as e:
                        logger.warning(
                            f"Error reading existing export file: {str(e)}",
                            extra={"markup": True},
                        )
                        logger.info(
                            "Will regenerate the export file", extra={"markup": True}
                        )

                # Extract work packages
                all_issues = work_package_migration.extract_issues_for_project(
                    jira_project_key, force=force
                )

                if not all_issues:
                    logger.warning(
                        f"No issues found for project {jira_project_key}",
                        extra={"markup": True},
                    )
                    project_results[jira_project_key] = {
                        "status": "warning",
                ...
                logger.info(
                    f"Extracted {len(all_issues)} issues for project {jira_project_key}",
                    extra={"markup": True},
                )
                ...
                logger.success(
                    f"Mapped {len(work_packages)} issues to work packages for project {jira_project_key}",
                    extra={"markup": True},
                )
                ...
                logger.success(
                    f"Exported {len(work_packages)} work packages to {export_file}",
                    extra={"markup": True},
                )
                ...
                progress.advance(
                    1, f"Exported {jira_project_key} ({len(work_packages)} WPs)"
                )

            except Exception as e:
                logger.error(
                    f"Error processing project {jira_project_key}: {str(e)}",
        ...
        logger.success(
            f"Export complete: {exported_projects}/{total_projects} projects, {total_work_packages} work packages",
            extra={"markup": True},
        )
...
def import_work_packages_to_rails(
    export_dir: str = None, project_key: str = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if not os.path.exists(export_dir):
            logger.error(
                f"Export directory not found: {export_dir}", extra={"markup": True}
            )
            return {
                "status": "error",
        ...
        container_name = migration_config.get("openproject", {}).get(
        ...
        if project_key:
            ...
            if os.path.exists(file_path):
                ...
                logger.error(
                    f"Export file not found for project {project_key}",
                    extra={"markup": True},
                )
                return {
                    "status": "error",
            ...
            for file_name in os.listdir(export_dir):
                if file_name.startswith("work_packages_") and file_name.endswith(
                    ".json"
                ):
        ...
        if not export_files:
            ...
            return {"status": "error", "message": "No export files found"}

        logger.info(
            f"Found {len(export_files)} export files to process", extra={"markup": True}
        )
        ...
        for export_file in export_files:
            file_name = os.path.basename(export_file)
            logger.notice(
                f"Processing export file: {file_name}", extra={"markup": True}
            )
            ...
            # Extract project key from filename
            current_project_key = file_name.replace("work_packages_", "").replace(
                ".json", ""
            )
        ...
        logger.success(
            f"Import complete: {total_created} work packages created, {total_errors} errors",
            extra={"markup": True},
        )
        logger.info(f"Summary report saved to {summary_file}", extra={"markup": True})
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Export Jira issues to JSON files for bulk import into OpenProject"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main():
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            logger.warning(
                "Operation completed with some issues", extra={"markup": True}
            )
        ...
        logger.error(
            f"Unexpected error: {str(e)}", extra={"markup": True, "traceback": True}
        )
        sys.exit(1)
