...
class AccountMigration(BaseMigration):
    ...
    def extract_tempo_accounts(self, *, force: bool = False) -> list:
        ...
        try:
            ...
            msg = f"Tempo API unauthorized or unavailable (401/403). Blocking migration: {e}"
    ...
    def associate_field_with_work_package_types(self, field_id: int) -> None:
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                "SUCCESS" in result
                or "=> nil" in result
                or (not any(error_word in result.lower() for error_word in ["error", "exception", "failed"]))
            ):
