...
class CustomFieldMigration(BaseMigration):
    ...
    def generate_ruby_script(self, fields_to_create: List[Dict[str, Any]]) -> str:
        ...
        """
        self.logger.info(f"Generating Ruby script for {len(fields_to_create)} custom fields...")
        ...
        self.logger.info(f"Generated Ruby script: {script_path}")
        return script_path

    def execute_ruby_script(self, script_path: str) -> bool:
        """
        Execute a Ruby script in OpenProject Rails console via SSH and Docker.
        ...
        if not op_server or not container_name:
            self.logger.error("Missing server or container configuration for automatic script execution")
            return False

        try:
            self.logger.info(f"Executing Ruby script via SSH and Docker on {op_server}...")

            # Modify the original script to collect custom field data at the end
            with open(script_path, 'r') as f:
                script_content = f.read()

            # Add a data collection script that returns results directly through output
            data_collection_script = """
# Collect data about custom fields for output
begin
  custom_fields = CustomField.where(type: "WorkPackageCustomField")
  puts "Found #{custom_fields.count} custom fields"

  output_data = custom_fields.map do |cf|
    {
      id: cf.id,
      name: cf.name,
      field_format: cf.field_format,
      status: cf.created_at > 1.hour.ago ? 'created' : 'existing',
      created_at: cf.created_at.to_s
    }
  end
  ...
  puts "Error collecting custom field data: #{e.message}"
  puts e.backtrace.join("\\n")
  { error: e.message, backtrace: e.backtrace }.to_json
end
"""
            # Append the data collection script to the original script
            script_content += "\n\n" + data_collection_script

            # Write the updated script to a temporary file
            import tempfile
            with tempfile.NamedTemporaryFile(mode='w', suffix='.rb', delete=False) as temp_script:
                temp_script.write(script_content)
                updated_script_path = temp_script.name
            ...
            # Execute the script using the Rails client
            self.logger.info("Executing script in Rails console")
            result = rails_client.execute_ruby_script(updated_script_path, in_container=False, output_as_json=True)
            ...
            if result.get('status') != 'success':
                self.logger.error(f"Script execution failed: {result.get('message', 'Unknown error')}")
                return False

            # Get the custom field data from the result
            custom_fields_data = result.get('data')
            if not custom_fields_data:
                self.logger.warning("No custom field data returned from script")
                return False

            self.logger.info(f"Successfully retrieved custom field data: {len(custom_fields_data)} fields")

            # Save the data to a local file
            local_result_path = os.path.join(self.output_dir, "custom_fields_result.json")
            with open(local_result_path, 'w') as f:
                json.dump(custom_fields_data, f, indent=2)

            self.logger.success(f"Saved custom field data to {local_result_path}")

            # Update the mapping
            update_count = 0
            for field_result in custom_fields_data:
                ...
                if field_name and field_id:
                    ...
                    for jira_id, field in self.mapping.items():
                        if field["openproject_name"] == field_name and field["matched_by"] == "create":
                            self.logger.info(f"Updating field '{field_name}' with ID {field_id}")
                            field["openproject_id"] = field_id
                            field["matched_by"] = "created" if field_status == "created" else "existing"
                            update_count += 1

            # Save the updated mapping
            if update_count > 0:
                self.logger.info(f"Updated {update_count} fields in mapping")
                self._save_to_json(self.mapping, "custom_field_mapping.json")
            else:
                self.logger.info("No fields needed updating in mapping")

            return True

        except Exception as e:
            self.logger.error(f"Error executing Ruby script: {str(e)}")
            import traceback
