...
class IssueTypeMigration(BaseMigration):
    ...
    def check_existing_work_package_types(self) -> list[dict[str, Any]]:
        ...
        try:
            self.logger.info(
                "Executing Rails command to write work package types to %s...",
                temp_file_path,
            )
            ...
            try:
                ...
                msg = f"Could not parse work package types JSON read from file: {e}"
                self.logger.exception(msg)
                self.logger.debug("Invalid JSON content: %s", json_content)
                raise MigrationError(msg) from e

        except subprocess.SubprocessError as e:
            msg = f"Error running docker exec command: {e}"
            self.logger.exception(msg)
            raise MigrationError(msg) from e
        except Exception as e:
            msg = f"Unexpected error during work package type retrieval: {e}"
            self.logger.exception(msg)
            raise MigrationError(msg) from e
        finally:
            try:
                if "container_name" in locals() and container_name:
                    self.logger.debug(
                        "Attempting final removal of remote temporary file %s...",
                        temp_file_path,
                    )
                    ssh_base_cmd = (
                        ["ssh", op_server, "--"]
    ...
    def migrate_issue_types_via_rails(self, window: int = 0, pane: int = 0) -> None:
        ...
        self.logger.info(
            "Saved %s work package types to create to %s",
            len(types_to_create),
            temp_file,
        )
        ...
        safe_container_results_path = ruby_escape(container_results_path)

        # 1. Header section with Python variable interpolation
        header_script = f"""
        # Ruby variables setup - generated by Python
        types_input_file = '{safe_container_temp_path}'
        results_output_file = '{safe_container_results_path}'
        """
        ...
        if "BULK_CREATE_FAILED" in output:
            ...
            msg = f"Bulk creation failed: {error_message}"
            self.logger.error(msg)
            raise MigrationError(msg)
        ...
        try:
            ...
            with results_file.open() as f:
                ...
                if not result_content.strip():
                    msg = f"Results file {results_file} is empty"
                    self.logger.error(msg)
                    self.logger.debug("Rails console output: %s", output)
                    raise MigrationError(msg)
                ...
                try:
                    ...
                    msg = f"Failed to parse results file content: {e}"
                    self.logger.exception(msg)
            ...
            self.logger.info(
                "Bulk creation completed: %s types created, %s errors",
                created_count,
                error_count,
            )
            ...
            for created_type in creation_results.get("created", []):
                ...
                if jira_type_name and jira_type_name in self.issue_type_mapping:
                    self.issue_type_mapping[jira_type_name]["openproject_id"] = (
                        created_type.get("id")
                    )
                    self.issue_type_mapping[jira_type_name]["matched_by"] = "created"
                    self.logger.info(
                        "Updated mapping for '%s' with ID %s",
                        jira_type_name,
                        created_type.get("id"),
                    )
            ...
            msg = f"Error processing creation results: {e}"
            self.logger.exception(msg)
            import traceback
