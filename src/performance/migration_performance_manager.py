...
class PerformanceConfig:
    ...
    def __post_init__(self):
        ...
        try:
            ...
            self.batch_size = SecurityValidator.validate_numeric_parameter('batch_size', self.batch_size)
            self.max_concurrent_batches = SecurityValidator.validate_numeric_parameter('max_concurrent_batches', self.max_concurrent_batches)
            self.batch_timeout = SecurityValidator.validate_numeric_parameter('batch_timeout', self.batch_timeout)
            
            # Validate rate limiting parameters
            self.max_requests_per_minute = SecurityValidator.validate_numeric_parameter('max_requests_per_minute', self.max_requests_per_minute)
            self.burst_size = SecurityValidator.validate_numeric_parameter('burst_size', self.burst_size)
            
            # Validate retry parameters
            self.max_retries = SecurityValidator.validate_numeric_parameter('max_retries', self.max_retries)
            ...
            self.progress_update_interval = SecurityValidator.validate_numeric_parameter('progress_update_interval', self.progress_update_interval)
            
            # Validate memory and performance parameters
            self.memory_limit_mb = SecurityValidator.validate_numeric_parameter('memory_limit_mb', self.memory_limit_mb)
            
            # Validate timing relationships
            SecurityValidator.validate_timing_relationships(self.base_delay, self.max_delay)
            ...
            estimated_max_workers = min(self.max_concurrent_batches, 8)  # Conservative estimate
            SecurityValidator.validate_resource_allocation(self.batch_size, estimated_max_workers, self.memory_limit_mb)
            
        except ConfigurationValidationError as e:
            logger.error(f"PerformanceConfig validation failed: {e}")
            raise
    ...
    def get_snapshot(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_memory = (
                ...
                if self._memory_measurements else 0.0
            )
...
class MigrationPerformanceManager:
    ...
    async def _process_with_batching(
        ...
        def rate_limited_processor(batch: List[T]) -> R:
            ...
            if self._shutdown_event.is_set():
                raise RuntimeError("Processing cancelled - manager shutting down")
    ...
    async def process_json_files_streaming_async(
        ...
        async def process_file_async(file_path: Path) -> List[Dict[str, Any]]:
            ...
            return await asyncio.to_thread(
                self._process_single_file, file_path, processor_func
            )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "throughput": {
                "items_per_second": (
                    snapshot["processed_items"] / total_time
                    if total_time > 0 else 0
                ),
                ...
                "success_rate": (
                    ...
                    if snapshot["total_items"] > 0 else 0
                )
            },
            "api_metrics": {
                ...
                "api_success_rate": (
                    ...
                    if snapshot["total_api_calls"] > 0 else 0
                )
            },
