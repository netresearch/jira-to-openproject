...
class ConfigurationManager:
    ...
    def create_config_from_template(
        ...
        if validate:
            ...
            if not validation_result.is_valid:
                raise ValueError(f"Generated configuration is invalid: {validation_result.errors}")
    ...
    def create_backup(self, config_path: Path, description: str = "") -> ConfigBackup:
        ...
        # Create backup file
        backup_filename = f"backup_{backup_id}_{timestamp.strftime('%Y%m%d_%H%M%S')}.yaml"
    ...
    def restore_backup(self, backup_id: str, target_path: Optional[Path] = None) -> Path:
    ...
    def import_config(self, config_content: str, format: str = "yaml") -> Dict[str, Any]:
        ...
        for template_file in self.templates_dir.glob("*.yaml.j2"):
            ...
            try:
                ...
                template = ConfigTemplate(
                    ...
                    environment=EnvironmentType(metadata.get('environment', 'development')),
    ...
    def list_backups(self) -> List[ConfigBackup]:
        ...
        for metadata_file in self.backups_dir.glob("*.meta.json"):
            try:
                ...
                print(f"Warning: Could not parse backup metadata {metadata_file}: {e}")
