...
class CredentialManager:
    ...
    def __init__(self, encryption_manager: EncryptionManager, vault_path: Path):
    ...
    def retrieve_credential(self, credential_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logging.error(f"Failed to decrypt credential {credential_id}: {e}")
            return None
...
class AuditLogger:
    ...
    def log_login_attempt(self, username: str, ip_address: str, 
        ...
        """Log a login attempt."""
        event = AuditEvent(
            event_type=AuditEventType.LOGIN if success else AuditEventType.SECURITY_EVENT,
            user_id=username,
    ...
    def log_migration_event(self, event_type: AuditEventType, migration_id: str,
                           user_id: str, details: Dict[str, Any], success: bool = True):
...
class AccessControlManager:
    ...
    def add_permission(self, user_role: UserRole, resource: str, action: str):
    ...
    def remove_permission(self, user_role: UserRole, resource: str, action: str):
        """Remove a permission for a user role."""
        if (user_role in self._permissions and 
            resource in self._permissions[user_role] and
            action in self._permissions[user_role][resource]):
...
class RateLimiter:
    ...
    async def is_allowed(self, client_id: str) -> Tuple[bool, Optional[datetime]]:
        ...
        async with self._lock:
            ...
            while (client_info.requests and 
                   (now - client_info.requests[0]).total_seconds() > self.window_seconds):
    ...
    async def get_client_stats(self, client_id: str) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {
                ...
                "blocked": client_info.blocked_until is not None and now < client_info.blocked_until,
                "blocked_until": client_info.blocked_until.isoformat() if client_info.blocked_until else None
            }
...
class SecurityManager:
    ...
    def get_session(self, session_id: str) -> Optional[UserSession]:
        ...
        if session and session.is_active:
            ...
            if ((datetime.now(UTC) - session.last_activity).total_seconds() > 
    ...
    async def check_permission(self, session_id: str, resource: str, action: str) -> bool:
    ...
    async def check_rate_limit(self, client_id: str) -> Tuple[bool, Optional[datetime]]:
...
def create_security_manager(config_path: Path = Path("config/security.yaml")) -> SecurityManager:
...
async def secure_api_call(security_manager: SecurityManager, session_id: str,
                         client_id: str, resource: str, action: str,
    ...
    # Check permissions
    has_permission = await security_manager.check_permission(session_id, resource, action)
...
def decrypt_config_value(encrypted_value: str, encryption_manager: EncryptionManager) -> str:
