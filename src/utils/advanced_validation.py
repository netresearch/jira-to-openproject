...
class ValidationSummary:
    ...
    phase_results: dict[ValidationPhase, list[ValidationResult]] = field(
...
class PreMigrationValidator(DataValidator):
    ...
    async def validate(
        ...
        if "openproject_config" in data:
            results.extend(
                await self._validate_openproject_config(
                    data["openproject_config"],
                    context,
                ),
            )
            ...
            if field_name not in project or not project[field_name]:
                results.append(
                    ValidationResult(
                        ...
                        message=f"Project missing required field: {field_name}",
                        entity_type="project",
            ...
            try:
                ...
                results.append(
                    ValidationResult(
                        phase=ValidationPhase.PRE_MIGRATION,
                        level=ValidationLevel.ERROR,
                        message=f"Invalid project key format: {e!s}",
                        entity_type="project",
    ...
    async def _validate_user(self, user: dict[str, Any]) -> list[ValidationResult]:
        ...
        for field_name in required_fields:
            if field_name not in user or not user[field_name]:
                results.append(
                    ValidationResult(
                        phase=ValidationPhase.PRE_MIGRATION,
                        level=ValidationLevel.ERROR,
                        message=f"User missing required field: {field_name}",
                        entity_type="user",
    ...
    async def _validate_openproject_config(
        self,
        config: dict[str, Any],
        context: dict[str, Any],
    ) -> list[ValidationResult]:
            ...
            if field_name not in config or not config[field_name]:
                results.append(
                    ValidationResult(
                        phase=ValidationPhase.PRE_MIGRATION,
                        level=ValidationLevel.CRITICAL,
                        message=f"OpenProject config missing required field: {field_name}",
    ...
    async def _validate_business_rules(
        ...
        if "issues" in data.get("jira_data", {}):
            results.extend(
                await self._check_orphaned_issues(
                    data["jira_data"]["issues"],
                    data["jira_data"].get("users", []),
                ),
            )
        ...
        if "projects" in data.get("jira_data", {}):
            results.extend(
                await self._check_circular_dependencies(data["jira_data"]["projects"]),
            )

        return results

    async def _check_orphaned_issues(
        self,
        ...
        for issue in issues:
            if issue.get("assignee") and issue["assignee"] not in user_usernames:
                results.append(
                    ValidationResult(
                        phase=ValidationPhase.PRE_MIGRATION,
                        level=ValidationLevel.WARNING,
...
class InFlightValidator(DataValidator):
            ...
            if field_name not in data or data[field_name] is None:
                results.append(
                    ValidationResult(
                        phase=ValidationPhase.IN_FLIGHT,
                        level=ValidationLevel.ERROR,
                        message=f"Required field missing after transformation: {field_name}",
                        entity_type=data.get("type", "unknown"),
                        entity_id=data.get("id") or data.get("key"),
                        field_name=field_name,
                    ),
    ...
    async def _validate_data_corruption(
        self,
        data: dict[str, Any],
        context: dict[str, Any],
    ) -> list[ValidationResult]:
            ...
            if self.entity_hashes[entity_id] != current_hash:
                results.append(
                    ValidationResult(
                        ...
                        entity_id=entity_id,
                        metadata={
                            "previous_hash": self.entity_hashes[entity_id],
...
class PostMigrationValidator(DataValidator):
    ...
    async def _compare_entity_data(
        self,
        source: dict[str, Any],
        target: dict[str, Any],
    ) -> list[ValidationResult]:
            ...
            if (
                ...
                results.append(
                    ValidationResult(
                        phase=ValidationPhase.POST_MIGRATION,
                        level=ValidationLevel.ERROR,
                        message=f"Data accuracy mismatch in field: {field_name}",
                        entity_type=target.get("type", "unknown"),
                        entity_id=target.get("id"),
                        field_name=field_name,
                        expected_value=source[field_name],
...
class ValidationFramework:
    ...
    async def run_in_flight_validation(
        self,
        data: dict[str, Any],
        context: dict[str, Any],
    ) -> ValidationSummary:
    ...
    async def _run_validation(
        ...
        for validator in self.validators[phase]:
            try:
                ...
                logger.exception(f"Validator {validator.name} failed: {e!s}")
    ...
    def get_validation_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "critical_errors": [
                r for r in self.validation_log if r.level == ValidationLevel.CRITICAL
            ],
            "errors": [
                r for r in self.validation_log if r.level == ValidationLevel.ERROR
            ],
            "warnings": [
                r for r in self.validation_log if r.level == ValidationLevel.WARNING
            ],
        }
