...
class ThreadSafeBatchProcessor[T, R]:
    ...
    def __init__(
        ...
        try:
            ...
            self.max_workers = SecurityValidator.validate_numeric_parameter(
                "max_workers",
                max_workers,
            )
            self.retry_attempts = SecurityValidator.validate_numeric_parameter(
                "retry_attempts",
                retry_attempts,
            )
            ...
            SecurityValidator.validate_resource_allocation(
                ...
                self.max_workers,
                2048,
            )  # 2GB memory limit for consistency
    ...
    def _process_batch_with_retry(
        ...
        if self._shutdown_event.is_set():
            msg = "Processor is shutting down"
        ...
        for attempt in range(self.retry_attempts + 1):
            try:
                ...
                if self._shutdown_event.is_set():
                    msg = "Processing cancelled - shutting down"
                    raise RuntimeError(msg)
    ...
    def process_parallel(
        ...
        if self._executor is None:
            msg = "BatchProcessor must be used as context manager"
            raise RuntimeError(msg)
        ...
        try:
            ...
            self.logger.exception(f"Parallel processing error: {e}")
            raise
            ...
            with open(file_path, "rb") as file:
                ...
                for _prefix, event, value in parser:
                    ...
                    try:
                        ...
                        failed_count += 1
                        errors.append(
                            f"Processing error at item {processed_count}: {e}",
                        )
                        self.logger.warning(
                            f"Failed to process item {processed_count}: {e}",
                        )
