...
class DataPreservationManager:
    ...
    def __init__(
        ...
        self.preservation_dir = preservation_dir or config.get_path("data").parent / "preservation"
    ...
    def _load_preservation_policies(self) -> None:
        ...
        policies_file = self.preservation_dir / "policies" / "preservation_policies.json"
                ...
                with policies_file.open() as f:
                    ...
                    for entity_type, policy in loaded_policies.items():
                        ...
                        if isinstance(policy.get("conflict_resolution"), str):
                            try:
                                converted_policy["conflict_resolution"] = ConflictResolution(
                                    policy["conflict_resolution"]
                                )
                            except ValueError:
                                self.logger.warning(
                                    "Invalid conflict_resolution value '%s' for %s, using default",
                                    policy["conflict_resolution"], entity_type
                                )
                                converted_policy["conflict_resolution"] = ConflictResolution.PROMPT_USER

                        # Convert merge_strategy string to enum
                        if isinstance(policy.get("merge_strategy"), str):
                            try:
                                converted_policy["merge_strategy"] = MergeStrategy(policy["merge_strategy"])
                            except ValueError:
                                self.logger.warning(
                                    "Invalid merge_strategy value '%s' for %s, using default",
                                    policy["merge_strategy"], entity_type
                                )
                                converted_policy["merge_strategy"] = MergeStrategy.LATEST_TIMESTAMP

                        converted_policies[entity_type] = converted_policy

                    # Merge with defaults, allowing overrides
                    self.preservation_policies = {**default_policies, **converted_policies}
    ...
    def _calculate_entity_checksum(self, entity_data: dict[str, Any]) -> str:
        ...
        volatile_fields = [
            'self', 'lastViewed', 'expand', 'transitions', 'operations',
            'editmeta', 'renderedFields', 'updated_on', 'last_activity_at'
        ]
    ...
    def store_original_state(
        ...
        try:
            ...
            self.logger.debug(
                ...
                entity_type, entity_id, checksum[:8]
            )
        except Exception as e:
            self.logger.warning(
    ...
    def detect_conflicts(
        ...
        resolution_strategy = policy["conflict_resolution"] if policy else ConflictResolution.PROMPT_USER
    ...
    def resolve_conflict(
        ...
        if resolution == ConflictResolution.JIRA_WINS:
            return self._apply_jira_wins_resolution(conflict, jira_data, openproject_data, policy)
        elif resolution == ConflictResolution.OPENPROJECT_WINS:
            return self._apply_openproject_wins_resolution(conflict, jira_data, openproject_data, policy)
        elif resolution == ConflictResolution.MERGE:
            return self._apply_merge_resolution(conflict, jira_data, openproject_data, policy)
    ...
    def _apply_openproject_wins_resolution(
        ...
        for field, value in jira_data.items():
            if field not in conflict["conflicted_fields"] and field not in policy["protected_fields"]:
                resolved_data[field] = value
    ...
    def _custom_merge_logic(self, field_name: str, jira_value: Any, openproject_value: Any) -> Any:
    ...
    def create_backup(
        ...
        try:
            ...
            self.logger.debug("Created backup for %s %s: %s", entity_type, entity_id, backup_file)
            return backup_file
        except Exception as e:
            self.logger.warning("Failed to create backup for %s %s: %s", entity_type, entity_id, e)
            raise
    ...
    def analyze_preservation_status(
        ...
        for entity_id, changes in jira_changes.items():
            ...
            try:
                ...
                current_op_data = self._get_openproject_entity_data(entity_id, entity_type)

                if current_op_data:
                    conflict = self.detect_conflicts(changes, entity_id, entity_type, current_op_data)
                    if conflict:
                        conflicts.append(conflict)
                        resolution = conflict["resolution_strategy"].value
                        conflicts_by_resolution[resolution] = conflicts_by_resolution.get(resolution, 0) + 1
    ...
    def _get_openproject_entity_data(self, entity_id: str, entity_type: str) -> dict[str, Any] | None:
    ...
    def update_preservation_policy(
        ...
        if entity_type not in self.preservation_policies:
            self.logger.warning("Unknown entity type for policy update: %s", entity_type)
            return
