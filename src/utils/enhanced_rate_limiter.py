...
class EnhancedRateLimiter:
    ...
    def _update_delay_metrics(self, delay: float):
    ...
    def handle_api_response(self, success: bool, headers: Dict[str, str] = None):
        ...
        with self._lock:
            if success:
                ...
                if (self._consecutive_successes >= 10 and 
                    self._current_max_requests < self.config.max_requests):
            ...
            if header.lower() in [k.lower() for k in headers.keys()]:
                try:
                    ...
                    if remaining < 5:  # Very low, be conservative
                        self._current_max_requests = max(1, self._current_max_requests // 2)
    ...
    def get_metrics(self) -> RateLimitMetrics:
        ...
        with self._lock:
            self.metrics.current_rate = (
                ...
                max(1, time.time() - self.metrics.last_reset_time)
            )
            return self.metrics
    ...
    def adjust_rate(self, new_max_requests: int):
        """Manually adjust the maximum request rate."""
        with self._lock:
            self.config.max_requests = new_max_requests
            self._current_max_requests = min(self._current_max_requests, new_max_requests)
...
class GlobalRateLimiterManager:
    ...
    def get_limiter(self, endpoint: str, config: RateLimitConfig = None) -> EnhancedRateLimiter:
    ...
    def remove_limiter(self, endpoint: str):
    ...
    def get_all_metrics(self) -> Dict[str, RateLimitMetrics]:
        ...
        with self._lock:
            return {endpoint: limiter.get_metrics() 
