...
def with_idempotency(
    header_extractor: Optional[Callable[..., Dict[str, str]]] = None,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            def execute_and_process():
                logger.debug("Executing function %s with idempotency key %s", func.__name__, idempotency_key)
                ...
                if result_processor:
                    try:
                        ...
                        logger.debug("Result processed successfully for key %s", idempotency_key)
                        return final_result
                    except Exception as e:
                        logger.warning("Result processor failed for key %s: %s", idempotency_key, e)
            ...
            # Use atomic get-or-set to handle race conditions
            cached_result = manager.atomic_get_or_set(idempotency_key, execute_and_process, ttl)
...
def batch_idempotent(ttl: Optional[int] = None):
...
def api_idempotent(ttl: Optional[int] = None):
...
def simple_idempotent(ttl: Optional[int] = None):
