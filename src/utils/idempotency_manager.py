...
class IdempotencyKeyManager:
    ...
    def _init_redis(self) -> None:
        ...
        try:
            ...
            # Load Lua script
            self._lua_script = self._redis_client.register_script(self.LUA_GET_SET_SCRIPT)
            ...
            logger.warning(
                "Redis connection failed, using in-memory fallback: %s", e
            )
            self._redis_available = False
    ...
    def _safe_json_loads(self, data: str) -> Any:
        ...
        try:
            ...
            if not isinstance(result, (dict, list, str, int, float, bool, type(None))):
                raise ValueError("Unsafe JSON object type")
    ...
    def atomic_get_or_set(
        ...
        if self._redis_available and self._redis_client and self._lua_script:
            try:
                ...
                try:
                    ...
                    logger.error("Failed to serialize result for caching: %s", e)
                ...
                try:
                    ...
                    if script_result == 1:  # Key was set (new)
                        with self._lock:
                            self._metrics["keys_cached"] += 1
                        return IdempotencyResult(found=False, value=result, source="redis")
                    else:  # Key already existed (cached)
                        with self._lock:
                            self._metrics["redis_hits"] += 1
        ...
        logger.debug("Using fallback atomic operation with lock for key %s", idempotency_key)
        with self._lock:
            existing = self.get_cached_result(idempotency_key)
            if existing.found:
                logger.debug("Found existing result in fallback cache for key %s", idempotency_key)
                return existing
            ...
            # Set the new value
            logger.debug("Setting new result in fallback cache for key %s", idempotency_key)
...
_idempotency_manager: Optional[IdempotencyKeyManager] = None
