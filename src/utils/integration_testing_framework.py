...
class TestEnvironmentManager:
    ...
    async def _setup_docker_environment(self) -> None:
        ...
        if not self.config.enable_docker:
            msg = "Docker environment requested but not enabled"
    ...
    async def _setup_real_environment(self) -> None:
        ...
        if not self.config.enable_real_apis:
            msg = "Real environment requested but not enabled"
            raise RuntimeError(msg)
    ...
    async def _start_jira_container(self) -> str:
        ...
        cmd = [
            ...
            "-e",
            "JIRA_OPTS=-Datlassian.license.message=disabled",
    ...
    async def _start_openproject_container(self) -> str:
        ...
        cmd = [
            ...
            "8081:80",
            "-e",
            "OPENPROJECT_SECRET_KEY_BASE=test_secret",
            "-e",
    ...
    async def _wait_for_service(self, url: str, service_name: str) -> None:
        ...
        msg = f"{service_name} failed to start within timeout"
        raise RuntimeError(msg)
    ...
    async def _generate_openproject_data(self) -> dict[str, Any]:
        ...
        for i in range(self.config.op_projects):
            ...
            project = {
                ...
                "description": {"raw": f"Test project {i+1} for integration testing"},
                "status": "on_track",
                "is_public": True,
            ...
            for j in range(self.config.op_work_packages):
                work_package = {
                    ...
                    "status": "New",
                    "priority": "Normal",
                    "author": {"id": 1, "name": "admin", "login": "admin"},
                    "assigned_to": {
    ...
    def _generate_custom_fields(self) -> list[dict[str, Any]]:
        ...
        for i in range(self.config.custom_fields):
            ...
            field = {
                ...
                "required": i % 3 == 0,
                "searcherKey": (
                    "textsearcher" if field_type == "text" else f"{field_type}searcher"
                ),
            }

            if field_type in ["select", "multiselect"]:
                field["allowedValues"] = [{"value": f"Option {j+1}"} for j in range(3)]
...
<body>
    ...
    def _generate_junit_report(self, output_file: Path) -> None:
        ...
        xml_content += (
            f'  <testsuite name="Integration Tests" tests="{len(self.reports)}">\n'
        )
        ...
        for report in self.reports:
            ...
            if report.result == TestResult.FAILED:
                ...
                xml_content += (
                    f'      <error message="{report.error_message or "Test error"}">\n'
                )
    ...
    def _generate_summary_report(self, output_file: Path) -> None:
        ...
        with open(output_file, "w") as f:
            ...
            for scope in TestScope:
                ...
                if scope_reports:
                    passed = sum(
                        1 for r in scope_reports if r.result == TestResult.PASSED
                    )
            ...
            for env in TestEnvironment:
                ...
                if env_reports:
                    passed = sum(
                        1 for r in env_reports if r.result == TestResult.PASSED
                    )
...
class IntegrationTestingFramework:
    ...
    async def run_single_test(self, test_name: str, test_func: Callable) -> TestReport:
        ...
        try:
            ...
            end_time = datetime.now(UTC)
            duration = (
                self.performance_monitor.stop_timer(test_name)
                if test_name in self.performance_monitor.start_times
                else 0
            )
...
if __name__ == "__main__":
    ...
    async def main() -> None:
        ...
        await framework.run_test_suite(suite)
