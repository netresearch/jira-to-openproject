...
class StalenessManager:
    ...
    def __init__(
        ...
        fallback_admin_user_id: str | None = None,
    ) -> None:
    ...
    def get_cached_mapping(self, user_key: str) -> dict[str, Any] | None:
        ...
        if entry.is_stale(self.refresh_interval):
            ...
            if refresh_result.fallback_applied:
    ...
    def _refresh_user_mapping(self, user_key: str) -> RefreshResult:
        ...
        for attempt in range(max_attempts):
            try:
                self.logger.debug(
                    "Refreshing user mapping: %s (attempt %d/%d)",
                    user_key,
                    attempt + 1,
                    max_attempts,
                )
                ...
                if user_data:
                    ...
                    if self._validate_user_data(user_data):
                        ...
                        self.logger.info(
                            "User mapping refreshed successfully: %s",
                            user_key,
                        )
                    ...
                    self.logger.warning(
                        "User validation failed for %s: user inactive or data mismatch",
                        user_key,
                    )
                ...
                if attempt < max_attempts - 1:
                    ...
                    self.logger.debug(
                        "No user data returned, retrying in %s seconds",
                        delay,
                    )
                    time.sleep(delay)
                    continue
        ...
        return self._apply_fallback_strategy(
            user_key,
            "Unexpected error in refresh",
            max_attempts,
        )

    def _get_jira_user_data(self, user_key: str) -> dict[str, Any] | None:
    ...
    def _apply_fallback_strategy(
        ...
        if self.fallback_strategy == FallbackStrategy.ASSIGN_ADMIN:
            return self._assign_admin_strategy(user_key, error_reason, attempts)
        if self.fallback_strategy == FallbackStrategy.CREATE_PLACEHOLDER:
    ...
    def _skip_strategy(
        ...
        self.logger.warning(
            "Skipping user mapping: %s (reason: %s)",
            user_key,
            error_reason,
        )
    ...
    def _assign_admin_strategy(
        ...
        self.add_entry(
            user_key,
            admin_user_data,
            {"fallback": True, "original_error": error_reason},
        )

        self.logger.info(
            "Reassigned user %s to admin %s (reason: %s)",
            user_key,
            self.fallback_admin_user_id,
            error_reason,
        )
    ...
    def _create_placeholder_strategy(
        self,
        user_key: str,
        error_reason: str,
        attempts: int,
