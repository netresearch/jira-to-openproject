...
class TestBatchIdempotencyIntegration:
    ...
    def test_batch_find_records_idempotency_cache_hit(self, mock_execute):
        ...
        result2 = self.client.batch_find_records(
            "User", [3, 4], headers=headers  # Different IDs but same key
        )
    ...
    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_batch_get_users_by_emails_idempotency(self, mock_execute):
        ...
        result2 = self.client.batch_get_users_by_emails(
            ["different@example.com"], headers=headers
        )
        assert mock_execute.call_count == 1  # No additional call
        assert result1 == result2

    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_batch_operations_different_keys(self, mock_execute):
    ...
    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_batch_operations_no_headers(self, mock_execute):
        ...
        result1 = self.client.batch_find_records("User", [1])
        result2 = self.client.batch_find_records("User", [1])
        
        # Should execute twice (no caching benefit)
        assert mock_execute.call_count == 2

    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_idempotency_with_retry_logic(self, mock_execute):
    ...
    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_partial_failure_idempotency(self, mock_execute):
    ...
    def test_concurrent_batch_operations_same_key(self):
        ...
        def worker(worker_id):
            try:
                with patch.object(self.client, 'execute_json_query', side_effect=mock_execute_query):
                    headers = {"X-Idempotency-Key": "concurrent-key"}
                    result = self.client.batch_find_records("User", [worker_id], headers=headers)
    ...
    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_performance_impact_measurement(self, mock_execute):
        ...
        result1 = self.client.batch_find_records("User", list(range(100)), headers=headers)
        first_call_time = time.time() - start_time
        
        # Second call - should be much faster (cached)
        start_time = time.time()
        result2 = self.client.batch_find_records("User", list(range(100)), headers=headers)
    ...
    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_batch_custom_fields_idempotency(self, mock_execute):
        ...
        result2 = self.client.batch_get_custom_fields_by_names(
            ["Different Field"], headers=headers
        )
        assert mock_execute.call_count == 1  # No additional call
        assert result1 == result2

    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_batch_projects_idempotency(self, mock_execute):
        ...
        # First call
        result1 = self.client.batch_get_projects_by_identifiers(identifiers, headers=headers)
        assert mock_execute.call_count == 1
        assert len(result1) == 2
        
        # Second call - should be cached
        result2 = self.client.batch_get_projects_by_identifiers(
            ["different-project"], headers=headers
        )
        assert mock_execute.call_count == 1  # No additional call
        assert result1 == result2

    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_large_batch_idempotency(self, mock_execute):
        ...
        result1 = self.client.batch_find_records(
            "User", list(range(1000)), headers=headers
        )
    ...
    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_idempotency_error_handling(self, mock_execute):
    ...
    def test_idempotency_ttl_behavior(self):
        ...
        with patch('src.utils.idempotency_manager.IdempotencyKeyManager.cache_result') as mock_cache:
    ...
    @patch('src.clients.openproject_client.OpenProjectClient.execute_json_query')
    def test_mixed_batch_operations_isolation(self, mock_execute):
        ...
        result2 = self.client.batch_get_users_by_emails(["user1@example.com"], headers=headers)
