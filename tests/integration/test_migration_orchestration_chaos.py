...
from src.clients.openproject_client import OpenProjectError, ConnectionError
...
class TestMigrationOrchestrationChaos:
    ...
    def migration_instance(self, mock_migration_components):
        ...
        with patch.multiple(
            ...
            migration.work_package_migration = mock_migration_components['work_package_migration']
    ...
    def test_jira_client_failure_during_initialization(self, migration_instance, mock_migration_components):
        """
        ...
        mock_migration_components['jira_client'].connect.side_effect = JiraError("Connection timeout")
        ...
        mock_migration_components['state_manager'].cleanup_failed_migration.assert_called_once()

    def test_openproject_client_failure_mid_migration(self, migration_instance, mock_migration_components):
        """
        ...
        mock_migration_components['openproject_client'].initialize.side_effect = ConnectionError("SSH connection lost")
        
        # Act & Assert
        with pytest.raises(MigrationError, match="Failed to initialize OpenProject client"):
            migration_instance.run_migration()
    ...
    @pytest.mark.parametrize("failure_point,expected_cleanup_calls", [
        ("user_migration", ["cleanup_users", "cleanup_projects", "cleanup_work_packages"]),
    ...
    def test_cascading_cleanup_on_migration_failure(
        self, migration_instance, mock_migration_components, failure_point, expected_cleanup_calls
    ):
        """
        ...
        getattr(mock_migration_components[failure_point], 'run').side_effect = MigrationError(f"{failure_point} failed")
    ...
    def test_random_component_failures_with_recovery(self, migration_instance, mock_migration_components):
        """
        ...
        for component_name in failure_components:
            ...
            component.connect = MagicMock(side_effect=[
                ConnectionError("Random failure") if random.random() < 0.3 else True,
    ...
    def test_state_corruption_recovery(self, migration_instance, mock_migration_components):
        """
        ...
        mock_migration_components['state_manager'].save_migration_state.side_effect = StateCorruptionError(
        ...
        mock_migration_components['state_manager'].attempt_state_recovery.assert_called_once()
        mock_migration_components['state_manager'].create_emergency_backup.assert_called_once()

    def test_concurrent_migration_conflict(self, migration_instance, mock_migration_components):
        """
        ...
        mock_migration_components['state_manager'].check_migration_lock.return_value = False
        mock_migration_components['state_manager'].acquire_migration_lock.side_effect = TimeoutError(
    ...
    def test_memory_pressure_scenario(self, migration_instance, mock_migration_components):
        """
        ...
        mock_migration_components['work_package_migration'].run.side_effect = MemoryError(
            "Cannot allocate memory for large dataset"
        )
        ...
        mock_migration_components['work_package_migration'].run_in_batches.assert_called()

    def test_network_partition_during_migration(self, migration_instance, mock_migration_components):
        """
    ...
    def test_resource_cleanup_on_interrupted_migration(self, migration_instance, mock_migration_components):
        """
        ...
        def simulate_interruption(*args, **kwargs):
            raise KeyboardInterrupt("Migration interrupted by user")
        
        mock_migration_components['work_package_migration'].run.side_effect = simulate_interruption
        ...
        mock_migration_components['state_manager'].save_interrupted_state.assert_called_once()
...
class TestClientLayerChaosIntegration:
    ...
    def test_ssh_connection_drops_during_operation(self, layered_client_stack):
        """
    ...
    def test_docker_container_becomes_unresponsive(self, layered_client_stack):
        """
        ...
        layered_client_stack['docker'].transfer_file_to_container.side_effect = TimeoutError(
            "Container unresponsive after 30 seconds"
        )
        
        # Act & Assert
        with pytest.raises(OpenProjectError, match="Container became unresponsive"):
            layered_client_stack['docker'].transfer_file_to_container("/tmp/local", "/tmp/remote")
        
        # Verify container restart was attempted
        layered_client_stack['docker'].restart_container.assert_called_once()

    def test_rails_console_memory_exhaustion(self, layered_client_stack):
        """
        ...
        with pytest.raises(OpenProjectError, match="Rails console memory exhaustion"):
            layered_client_stack['rails'].execute("WorkPackage.all.includes(:attachments).to_json")
