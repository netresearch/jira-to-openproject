...
from src.clients.openproject_client import OpenProjectClient, ConnectionError, RateLimitError
from src.clients.jira_client import JiraClient, JiraError
...
class TestMemoryPressureScenarios:
    ...
    def test_memory_efficient_batch_processing(self, memory_constrained_migration, large_dataset):
        """
    ...
    def test_memory_pressure_triggers_garbage_collection(self, memory_constrained_migration):
        """
        ...
        with patch('gc.collect') as mock_gc:
            ...
            memory_constrained_migration.process_large_batch([{'id': i} for i in range(100)])
            
            # Assert: Garbage collection should be triggered
            mock_gc.assert_called()

    def test_out_of_memory_error_recovery(self, memory_constrained_migration):
        """
...
class TestRateLimitingBoundaries:
    ...
    def test_jira_api_rate_limiting_with_backoff(self, rate_limited_client):
        """
    ...
    def test_openproject_api_rate_limiting_batch_adjustment(self, rate_limited_client):
        """
        ...
        def mock_create_batch(items):
            if len(items) > 10:
                raise RateLimitError("Batch too large for rate limit")
    ...
    def test_concurrent_api_calls_respect_rate_limits(self, rate_limited_client):
        """
        ...
        def mock_api_call_with_rate_limiting(*args, **kwargs):
            ...
            if len(recent_calls) > max_calls_per_window:
                raise RateLimitError("Too many concurrent calls")
...
class TestNetworkResilienceAndRecovery:
    ...
    def test_ssh_connection_recovery_after_network_partition(self, network_aware_migration):
        """
        ...
        result = network_aware_migration.openproject_client.execute_with_retry("ls /tmp")
    ...
    def test_docker_container_communication_timeout_handling(self, network_aware_migration):
        """
        ...
        def mock_container_exec(command):
            nonlocal timeout_count
            timeout_count += 1
            
            # First 2 calls timeout
            if timeout_count <= 2:
                raise TimeoutError(f"Container operation timed out after {network_aware_migration.network_timeout}s")
        ...
        result = network_aware_migration.openproject_client.execute_in_container("rails console")
        
        # Assert: Should succeed after retries
        assert result == "container response"
        assert timeout_count == 3

    def test_rails_console_session_recovery_after_disconnect(self, network_aware_migration):
        """
        ...
        def mock_execute_command(command):
            ...
            if len(session_attempts) == 1:
                raise ConnectionError("Rails console session lost")
        ...
        result = network_aware_migration.openproject_client.execute_rails_command("User.count")
...
class TestConcurrentResourceContention:
    ...
    def test_file_system_resource_contention(self):
        """
        ...
        def mock_close_file(file_handle):
        ...
        def perform_file_operations(op_id):
            try:
                files = []
                for i in range(3):
        ...
        successful_ops = [op for op in file_operations if "success" in op]
