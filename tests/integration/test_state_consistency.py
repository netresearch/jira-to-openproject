...
class TestStateConsistencyUnderConcurrency:
    ...
    def test_concurrent_state_writes_maintain_consistency(
        self,
        state_manager,
        ...
        def write_state(migration) -> None:
            for i in range(5):
                state_data = {
                    "step": i,
                    "progress": i * 20,
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(write_state, migration)
    ...
    def test_migration_record_snapshot_consistency(self, state_manager) -> None:
        ...
        with pytest.raises(
            ...
            match="Migration record exists but snapshot is corrupted",
        ):
    ...
    def test_partial_failure_state_preservation(self, state_manager) -> None:
        ...
        assert (
            migration_state["steps"]["work_packages"]["migrated_count"] == 500
        )  # Partial progress preserved
        assert "Network timeout" in migration_state["steps"]["work_packages"]["error"]

    def test_concurrent_migration_lock_contention(self, state_manager) -> None:
        ...
        def simulate_lock_acquisition(migration_id) -> None:
            try:
                # Attempt to acquire lock with timeout
                acquired = state_manager.acquire_migration_lock(
                    migration_id,
                    timeout=2.0,
                )
                if acquired:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(simulate_lock_acquisition, mid) for mid in migration_ids
            ]
            for future in futures:
                future.result()
        ...
        successful_acquisitions = [
            result
            for result in lock_acquisition_results.values()
    ...
    def test_state_recovery_from_corrupted_snapshot(self, state_manager) -> None:
        ...
        assert "projects" in [
            step["step"] for step in recovered_state["completed_steps"]
        ]

    def test_atomic_state_transitions(self, state_manager) -> None:
    ...
    def test_concurrent_snapshot_creation(self, state_manager) -> None:
        ...
        def create_snapshot(migration_id) -> None:
            try:
                snapshot_data = {
                    "migration_id": migration_id,
                ...
                snapshot_results[migration_id] = {"status": "error", "error": str(e)}
...
class TestDataIntegrityDuringFailures:
    ...
    def test_rollback_on_validation_failure(self, migration_with_state) -> None:
    ...
    def test_referential_integrity_preservation(self, migration_with_state) -> None:
        ...
        def check_references(data) -> bool:
            for wp in data:
                if wp.get("project_id") == 999:
                    msg = f"Invalid project_id {wp['project_id']} for work package {wp['id']}"
                    raise Exception(
                        msg,
                    )
            return True
        ...
        assert (
            migration_with_state.state_manager.get_migrated_count("work_packages") == 0
        )
