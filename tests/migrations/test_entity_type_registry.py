...
class TestEntityTypeRegistry:
    ...
    def test_register_multiple_classes(self):
        ...
        EntityTypeRegistry.register(MockWorkPackageMigration, ["work_packages", "issues"])
    ...
    def test_get_supported_types_preserves_order(self):
        ...
        # Act
        supported_types = EntityTypeRegistry.get_supported_types(MockWorkPackageMigration)

        # Assert
        assert supported_types == entity_types

    def test_get_class_for_type_reverse_lookup(self):
    ...
    def test_get_all_registered_types(self):
        """Test getting all registered types across all classes."""
        # Arrange
        EntityTypeRegistry.register(MockUserMigration, ["users", "accounts"])
        EntityTypeRegistry.register(MockProjectMigration, ["projects", "accounts"])  # Duplicate
...
class TestEntityTypeRegistryErrorHandling:
    ...
    def test_register_with_empty_entity_types_raises_error(self):
    ...
    def test_register_with_none_entity_types_raises_error(self):
        """Test registration with None entity types."""
        with pytest.raises(ValueError, match="must support at least one entity type"):
            EntityTypeRegistry.register(MockUserMigration, None)

    def test_register_non_base_migration_class_raises_error(self):
    ...
    def test_register_with_none_class_raises_error(self):
    ...
    def test_resolve_unregistered_class_raises_error(self):
        """Test resolving unregistered migration class."""
        with pytest.raises(ValueError, match="is not registered with EntityTypeRegistry"):
            EntityTypeRegistry.resolve(MockUserMigration)

    def test_resolve_none_class_raises_error(self):
        """Test resolving None class."""
        with pytest.raises(ValueError, match="Migration class cannot be None"):
            EntityTypeRegistry.resolve(None)

    def test_get_supported_types_unregistered_class_raises_error(self):
        """Test get_supported_types for unregistered class."""
        with pytest.raises(ValueError, match="is not registered with EntityTypeRegistry"):
            EntityTypeRegistry.get_supported_types(MockUserMigration)

    def test_register_duplicate_entity_type_logs_warning(self, caplog):
...
class TestRegisterEntityTypesDecorator:
    ...
    def test_decorator_registers_class_on_definition(self):
    ...
    def test_decorator_with_single_entity_type(self):
        ...
        assert EntityTypeRegistry.get_supported_types(SingleTypeMigration) == ["single_type"]

    def test_decorator_with_multiple_entity_types(self):
        ...
        assert EntityTypeRegistry.get_supported_types(MultiTypeMigration) == ["type1", "type2", "type3"]
    ...
    def test_decorator_with_no_entity_types_raises_error(self):
...
class TestBaseMigrationIntegration:
    ...
    def test_auto_detect_entity_type_success(self):
    ...
    def test_auto_detect_entity_type_unregistered_logs_warning(self, caplog):
    ...
    def test_auto_detect_preserves_exception_details(self, caplog):
...
class TestEntityTypeRegistryEdgeCases:
    ...
    def test_register_same_class_multiple_times(self):
        """Test registering same class multiple times updates registration."""
        # Arrange & Act
        EntityTypeRegistry.register(MockUserMigration, ["users"])
        EntityTypeRegistry.register(MockUserMigration, ["users", "accounts", "profiles"])

        # Assert
        assert EntityTypeRegistry.resolve(MockUserMigration) == "users"
        assert EntityTypeRegistry.get_supported_types(MockUserMigration) == ["users", "accounts", "profiles"]

    def test_entity_type_with_special_characters(self):
        """Test entity types with special characters."""
        # Arrange & Act
        EntityTypeRegistry.register(MockUserMigration, ["user-accounts", "user_profiles", "user.data"])
...
class TestEntityTypeRegistryConcurrency:
    ...
    def test_concurrent_registration(self):
        ...
        def register_unique_class(thread_id: int):
    ...
    def test_concurrent_access_different_operations(self):
        ...
        def read_operations():
            ...
            try:
                ...
                results.append(EntityTypeRegistry.get_supported_types(MockProjectMigration))
...
class TestEntityTypeRegistryRealWorldScenarios:
    ...
    def test_typical_migration_setup(self):
        ...
        all_types = EntityTypeRegistry.get_all_registered_types()
        expected_types = {
            "users", "user_accounts", "projects", "work_packages", 
            "issues", "issue_types", "work_package_types"
        }
    ...
    def test_migration_orchestrator_usage_pattern(self):
    ...
    def test_migration_class_inheritance_hierarchy(self):
