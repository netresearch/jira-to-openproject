...
import pytest
import importlib
from src.migrations.base_migration import EntityTypeRegistry, BaseMigration
...
@pytest.mark.parametrize("module_path, class_name, expected_types", MIGRATION_CLASSES_TO_TEST)
def test_migration_class_is_registered_on_import(module_path: str, class_name: str, expected_types: list[str]):
    """
...
class TestMigrationClassRegistrationIntegration:
    ...
    def test_all_classes_register_different_primary_types(self):
    ...
    def test_auto_detect_entity_type_integration(self):
    ...
    def test_registry_lookup_completeness(self):
...
class TestMigrationClassRegistrationErrorScenarios:
    ...
    def test_unregistered_class_behavior(self):
    ...
    def test_multiple_entity_types_registration(self):
        """Verify that classes with multiple entity types register all of them correctly."""
        # Focus on classes with multiple entity types
        multi_type_classes = [
            ("src.migrations.account_migration", "AccountMigration", ["accounts", "tempo_accounts"]),
            ("src.migrations.status_migration", "StatusMigration", ["statuses", "status_types"]),
        ]
        ...
        for module_path, class_name, expected_types in multi_type_classes:
            ...
            primary_type = EntityTypeRegistry.resolve(migration_class)
            assert primary_type == expected_types[0], \
