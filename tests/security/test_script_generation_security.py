...
def _verify_script_safety(script, original_jira_key):
    ...
    for pattern in dangerous_patterns:
        assert pattern not in script_content.upper(), \
            f"Dangerous pattern '{pattern}' found in script"


def _verify_script_has_no_dangerous_patterns(script):
...
def test_author_script_valid_key_is_escaped(ua_migrator, clean_cache):
    ...
    assert f"WorkPackage.find(42)" in script
...
@pytest.mark.integration
@pytest.mark.security  
def test_author_script_malicious_key_rejected_before_generation(ua_migrator, clean_cache):
    ...
    for malicious_key in malicious_keys:
        ...
        ua_migrator._queue_rails_author_operation(
            malicious_key, author_id=7, author_data={}
        )
...
@pytest.mark.integration
@pytest.mark.security
def test_author_script_multiple_operations_all_escaped(ua_migrator, clean_cache):
    ...
    for i, (jira_key, author_id) in enumerate(operations):
        ...
        ua_migrator._queue_rails_author_operation(jira_key, author_id=author_id, author_data={})
...
@pytest.mark.integration
@pytest.mark.security
def test_author_script_no_mapping_safe_handling(ua_migrator, clean_cache):
...
@pytest.mark.integration
@pytest.mark.security
def test_timestamp_script_valid_key_is_escaped(ts_migrator, clean_cache):
...
@pytest.mark.integration
@pytest.mark.security
def test_timestamp_script_malicious_key_rejected(ts_migrator, clean_cache):
...
@pytest.mark.integration
@pytest.mark.security
def test_timestamp_script_field_name_also_escaped(ts_migrator, clean_cache):
...
@pytest.mark.integration
@pytest.mark.security
def test_empty_operations_cache_safe(ua_migrator, ts_migrator):
...
def test_script_generation_deterministic_with_mocked_time(mock_datetime, ua_migrator, clean_cache):
...
@pytest.mark.integration
@pytest.mark.security
def test_large_volume_operations_performance_and_safety(ua_migrator, clean_cache):
    ...
    for i in range(num_operations):
        ...
        wp_id = 1000 + i
        wp_map[wp_id] = {"jira_key": jira_key, "openproject_id": wp_id}
        ua_migrator._queue_rails_author_operation(jira_key, author_id=i % 10, author_data={})
...
@pytest.mark.integration
@pytest.mark.security
def test_unicode_in_valid_keys_handled_safely(ua_migrator, clean_cache):
