...
class TestAccountMigration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.tempo_accounts = [
            {'id': '101', 'key': 'ACCT1', 'name': 'Account One', 'companyId': '201', 'default_project': {'key': 'PROJ1'}},
            {'id': '102', 'key': 'ACCT2', 'name': 'Account Two', 'companyId': '202', 'default_project': {'key': 'PROJ2'}},
        ]

        # Sample OpenProject projects data
        self.op_projects = [
            {'id': 1, 'name': 'Account One', 'identifier': 'account-one', '_links': {'parent': {'href': '/api/v3/projects/10'}}},
            {'id': 3, 'name': 'Some Other Project', 'identifier': 'some-other-project', '_links': {}},
        ]
    ...
    def test_extract_tempo_accounts(self, mock_file, mock_exists, mock_get_path,
                                    mock_rails_client, mock_op_client, mock_jira_client):
        ...
        migration = AccountMigration(mock_jira_instance, mock_op_instance, mock_rails_instance)
    ...
    @patch('os.path.exists')
    @patch('builtins.open', new_callable=mock_open)
    def test_create_account_mapping(self, mock_file, mock_exists, mock_migration_config, mock_get_path,
                                    mock_rails_client, mock_op_client, mock_jira_client):
        ...
        project_mapping_mock = mock_open(read_data=json.dumps({
    ...
    @patch('os.path.exists')
    @patch('builtins.open', new_callable=mock_open)
    def test_create_account_custom_field(self, mock_file, mock_exists, mock_migration_config,
                                         mock_get_path, mock_rails_client, mock_op_client, mock_jira_client):
        ...
        with patch.object(migration, '_save_custom_field_id'):
            ...
            mock_rails_instance.get_custom_field_id_by_name.assert_called_once_with('Tempo Account')
    ...
    @patch('os.path.exists')
    @patch('builtins.open', new_callable=mock_open)
    def test_migrate_accounts(self, mock_file, mock_exists, mock_tracker, mock_migration_config,
                            mock_get_path, mock_rails_client, mock_op_client, mock_jira_client):
        ...
        with patch.object(migration, '_save_to_json', side_effect=save_to_json_side_effect):
