...
class TestCustomFieldMigration(unittest.TestCase):
    ...
    def test_generate_ruby_script(self):
        """Test generating a Ruby script for custom field creation."""
        fields_to_create = [
            {
                "jira_id": "customfield_10001",
                "jira_name": "Test Text Field",
        ...
        # Patch datetime.now for consistent output path
        with patch('src.migrations.custom_field_migration.datetime') as mock_datetime:
            mock_datetime.now.return_value.strftime.return_value = "20230515_120000"

            # Run the method to generate a script
            script_path = self.migration.generate_ruby_script(fields_to_create)

            # Get the script content that would have been written to the file
            mock_file = self.mock_open.return_value.__enter__.return_value
            script_content = "".join([call_args[0][0] for call_args in mock_file.write.call_args_list])

            # Verify the script contains expected content
            self.assertIn("# Generated by Jira to OpenProject migration script", script_content)
            self.assertIn("Test Text Field", script_content)
            self.assertIn("Test List Field", script_content)
            self.assertIn("Option 1", script_content)
