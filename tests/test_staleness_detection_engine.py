...
class MockJiraClient:
    ...
    def __init__(self, users=None, raise_exception=False):
...
class MockOpenProjectClient:
    ...
    def __init__(self, users_by_email=None, raise_exception=False):
    ...
    def get_user_by_email(self, email):
        ...
        if self._raise_exception:
            raise RuntimeError("OpenProject API error")
...
def mock_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.utils.enhanced_user_association_migrator.config.get_path", 
                       lambda x: tmp_path)
    monkeypatch.setattr("src.utils.enhanced_user_association_migrator.config.logger",
                       MagicMock())
    ...
    monkeypatch.setattr("src.utils.enhanced_user_association_migrator.config.migration_config",
                       mock_migration_config)
...
class TestStaleMappingError:
    ...
    def test_exception_initialization_default_reason(self):
    ...
    def test_exception_initialization_custom_reason(self):
...
class TestStalenessDetection:
    ...
    def test_is_mapping_stale_invalid_timestamp(self, basic_migrator, caplog):
    ...
    def test_is_mapping_stale_custom_current_time(self, basic_migrator):
...
class TestStalenessCheckHandling:
    ...
    def test_check_fresh_mapping_returns_mapping(self, migrator_with_mappings):
        """Test fresh mapping is returned."""
        migrator = migrator_with_mappings
        mapping = migrator.check_and_handle_staleness("fresh_user", raise_on_stale=False)
        
        assert mapping is not None
        assert mapping["jira_username"] == "fresh_user"
    
    def test_check_missing_mapping_raises_exception(self, basic_migrator):
    ...
    def test_check_missing_mapping_returns_none(self, basic_migrator):
        """Test missing mapping returns None when raise_on_stale=False."""
        migrator = basic_migrator
        mapping = migrator.check_and_handle_staleness("nonexistent", raise_on_stale=False)
        
        assert mapping is None
    
    def test_check_stale_mapping_raises_exception(self, migrator_with_mappings):
    ...
    def test_check_stale_mapping_returns_none(self, migrator_with_mappings):
        """Test stale mapping returns None when raise_on_stale=False."""
        migrator = migrator_with_mappings
        mapping = migrator.check_and_handle_staleness("stale_user", raise_on_stale=False)
...
class TestMappingRetrievalWithStalenessCheck:
    ...
    def test_get_fresh_mapping_returns_immediately(self, migrator_with_mappings):
        """Test fresh mapping is returned without refresh attempt."""
        migrator = migrator_with_mappings
        
        with patch.object(migrator, 'refresh_user_mapping') as mock_refresh:
            mapping = migrator.get_mapping_with_staleness_check("fresh_user", auto_refresh=True)
            
            assert mapping is not None
            assert mapping["jira_username"] == "fresh_user"
            mock_refresh.assert_not_called()
    
    def test_get_stale_mapping_no_auto_refresh(self, migrator_with_mappings):
        """Test stale mapping returns None when auto_refresh=False."""
        migrator = migrator_with_mappings
        
        with patch.object(migrator, 'refresh_user_mapping') as mock_refresh:
            mapping = migrator.get_mapping_with_staleness_check("stale_user", auto_refresh=False)
            
            assert mapping is None
            mock_refresh.assert_not_called()
    
    def test_get_stale_mapping_auto_refresh_success(self, migrator_with_mappings):
        ...
        with patch.object(migrator, 'refresh_user_mapping', return_value=refreshed_mapping) as mock_refresh:
            mapping = migrator.get_mapping_with_staleness_check("stale_user", auto_refresh=True)
            
            assert mapping is refreshed_mapping
            mock_refresh.assert_called_once_with("stale_user")
    
    def test_get_stale_mapping_auto_refresh_fails(self, migrator_with_mappings):
        """Test stale mapping with failed auto-refresh."""
        migrator = migrator_with_mappings
        
        with patch.object(migrator, 'refresh_user_mapping', return_value=None) as mock_refresh:
            mapping = migrator.get_mapping_with_staleness_check("stale_user", auto_refresh=True)
            
            assert mapping is None
            mock_refresh.assert_called_once_with("stale_user")
    
    def test_get_mapping_exception_handling(self, migrator_with_mappings):
        """Test exception handling in staleness check."""
        migrator = migrator_with_mappings
        
        with patch.object(migrator, 'check_and_handle_staleness', side_effect=Exception("Unexpected error")):
...
class TestBulkStalenessDetection:
    ...
    def test_detect_stale_mappings_all_users(self, migrator_with_mappings):
    ...
    def test_detect_stale_mappings_specific_users(self, migrator_with_mappings):
    ...
    def test_detect_stale_mappings_no_stale_users(self, basic_migrator):
    ...
    def test_detect_stale_mappings_error_handling(self, basic_migrator):
        ...
        with patch.object(migrator, 'is_mapping_stale', side_effect=Exception("Test error")):
            stale_mappings = migrator.detect_stale_mappings()
...
class TestBatchRefresh:
    ...
    def test_batch_refresh_no_stale_mappings(self, basic_migrator):
    ...
    def test_batch_refresh_success_first_try(self, basic_migrator):
        ...
        with patch.object(migrator, 'detect_stale_mappings', return_value=stale_mappings), \
             patch.object(migrator, 'refresh_user_mapping', return_value=refreshed_mapping):
    ...
    def test_batch_refresh_retry_then_success(self, basic_migrator):
    ...
    def test_batch_refresh_all_retries_fail(self, basic_migrator):
    ...
    def test_batch_refresh_exception_handling(self, basic_migrator):
        ...
        with patch.object(migrator, 'detect_stale_mappings', return_value=stale_mappings), \
             patch.object(migrator, 'refresh_user_mapping', side_effect=Exception("API error")):
...
class TestIndividualRefresh:
    ...
    def test_refresh_user_mapping_success(self, basic_migrator):
    ...
    def test_refresh_user_mapping_user_not_found(self, basic_migrator):
    ...
    def test_refresh_user_mapping_inactive_user(self, basic_migrator):
    ...
    def test_refresh_user_mapping_api_error(self, basic_migrator):
...
class TestOpenProjectMappingAttempt:
    ...
    def test_attempt_openproject_mapping_success(self, basic_migrator):
    ...
    def test_attempt_openproject_mapping_no_match(self, basic_migrator):
    ...
    def test_attempt_openproject_mapping_no_email(self, basic_migrator):
...
class TestManualRefreshTrigger:
    ...
    def test_trigger_refresh_fresh_mapping_no_force(self, migrator_with_mappings):
    ...
    def test_trigger_refresh_fresh_mapping_with_force(self, migrator_with_mappings):
    ...
    def test_trigger_refresh_stale_mapping_success(self, migrator_with_mappings):
    ...
    def test_trigger_refresh_failure(self, migrator_with_mappings):
    ...
    def test_trigger_refresh_exception(self, basic_migrator):
...
class TestMappingValidation:
    ...
    def test_validate_mapping_freshness_all_users(self, migrator_with_mappings):
    ...
    def test_validate_mapping_freshness_specific_users(self, migrator_with_mappings):
    ...
    def test_validate_mapping_recommendations(self, migrator_with_mappings):
    ...
    def test_validate_mapping_error_handling(self, basic_migrator):
...
class TestConfigurationParsing:
    ...
    def test_parse_duration_valid_formats(self, duration_str, expected_seconds):
    ...
    def test_validate_fallback_strategy_valid(self, strategy):
    ...
    def test_validate_fallback_strategy_invalid(self, invalid_strategy):
        """Test validation of invalid fallback strategies."""
        migrator = EnhancedUserAssociationMigrator
        with pytest.raises(ValueError):
            migrator._validate_fallback_strategy(None, invalid_strategy)
    
    def test_load_staleness_config_defaults(self, basic_migrator):
...
class TestMigrationIntegration:
    ...
    def test_migrate_assignee_with_staleness_detection(self, basic_migrator):
        ...
        with patch.object(migrator, 'get_mapping_with_staleness_check', return_value=fresh_mapping) as mock_check:
    ...
    def test_migrate_assignee_stale_mapping_fallback(self, basic_migrator):
        ...
        with patch.object(migrator, 'get_mapping_with_staleness_check', side_effect=mock_staleness_check):
            result = migrator._migrate_assignee(assignee_data, work_package_data)
            
            assert work_package_data["assigned_to_id"] == 999  # Fallback user
            assert any("mapping stale" in w for w in result["warnings"])
    
    def test_migrate_author_with_staleness_detection(self, basic_migrator):
        ...
        with patch.object(migrator, 'get_mapping_with_staleness_check', return_value=fresh_mapping) as mock_check:
            result = migrator._migrate_author(reporter_data, None, work_package_data, False)
            
            mock_check.assert_called_once_with("test_author", auto_refresh=True)
            assert work_package_data["author_id"] == 456
            assert len(result["warnings"]) == 0
    
    def test_migrate_watchers_with_staleness_detection(self, basic_migrator):
        ...
        with patch.object(migrator, 'get_mapping_with_staleness_check', side_effect=mock_staleness_check) as mock_check:
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_unicode_usernames(self, basic_migrator):
        ...
        mapping = migrator.check_and_handle_staleness(unicode_username, raise_on_stale=False)
        assert mapping is None
    
    def test_very_large_mapping_dataset(self, basic_migrator):
    ...
    def test_concurrent_modifications(self, basic_migrator):
        """Test handling of concurrent mapping modifications."""
        migrator = basic_migrator
        
        # Simulate concurrent modification during staleness check
        original_mappings = migrator.enhanced_user_mappings.copy()
        
        def modify_during_check(*args, **kwargs):
        ...
        with patch.object(migrator, 'is_mapping_stale', side_effect=modify_during_check):
            # Should handle concurrent modification gracefully
            stale_mappings = migrator.detect_stale_mappings()
            assert isinstance(stale_mappings, dict)
    
    def test_memory_efficiency_batch_operations(self, basic_migrator):
