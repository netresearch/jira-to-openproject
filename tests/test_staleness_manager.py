...
class TestCacheEntry:
    ...
    def test_cache_entry_age_calculation(self) -> None:
        ...
        entry = CacheEntry(mapped_user={"accountId": "123"}, last_refreshed=past_time)

        age = entry.age()
        assert age >= timedelta(hours=1, minutes=59)  # Allow some test execution time
        assert age <= timedelta(hours=2, minutes=1)

    def test_cache_entry_staleness_detection(self) -> None:
        ...
        none_entry = CacheEntry(mapped_user=None, last_refreshed=datetime.now(tz=UTC))
...
class TestStalenessManager:
    ...
    def test_initialization(self, mock_jira_client, mock_op_client) -> None:
    ...
    def test_duration_parsing(self, staleness_manager) -> None:
    ...
    def test_cache_miss(self, staleness_manager) -> None:
    ...
    def test_cache_hit_fresh_data(self, staleness_manager) -> None:
    ...
    def test_cache_hit_stale_data_successful_refresh(self, staleness_manager) -> None:
        ...
        staleness_manager.cache["test_user"].last_refreshed = datetime.now(
            tz=UTC,
        ) - timedelta(hours=2)
    ...
    def test_cache_hit_stale_data_refresh_fails_skip_strategy(
        self,
        staleness_manager,
    ) -> None:
        ...
        # Mock failed refresh
        staleness_manager.jira_client.get_user_info.side_effect = JiraApiError(
            "User not found",
        )
    ...
    def test_user_validation_missing_fields(self, staleness_manager) -> None:
    ...
    def test_user_validation_success(self, staleness_manager) -> None:
    ...
    def test_assign_admin_fallback_strategy(
        self,
        mock_jira_client,
        mock_op_client,
    ) -> None:
        ...
        manager.cache["test_user"].last_refreshed = datetime.now(tz=UTC) - timedelta(
            hours=2,
        )
    ...
    def test_assign_admin_fallback_no_admin_configured(
        self,
        mock_jira_client,
        mock_op_client,
    ) -> None:
    ...
    def test_create_placeholder_fallback_strategy(
        self,
        mock_jira_client,
        mock_op_client,
    ) -> None:
    ...
    def test_exponential_backoff_retry(self, mock_sleep, staleness_manager) -> None:
    ...
    def test_cache_stats(self, staleness_manager) -> None:
    ...
    def test_clear_cache(self, staleness_manager) -> None:
        """Test cache clearing."""
    ...
    def test_get_jira_user_data_exception_handling(self, staleness_manager) -> None:
        """Test that get_jira_user_data handles exceptions gracefully."""
        staleness_manager.jira_client.get_user_info.side_effect = Exception(
            "Network error",
        )
