...
class TestConfigLoaderSecurityEnhanced:
    ...
    def test_database_config_no_credential_exposure_in_logs(self, mock_load_yaml, mock_apply_env, mock_load_dotenv, mock_config_data, caplog):
    ...
    def test_docker_secret_no_credential_exposure_in_logs(self, mock_path, mock_load_yaml, mock_apply_env, mock_load_dotenv, mock_config_data, caplog):
    ...
    def test_docker_secret_permission_error_no_credential_exposure(self, mock_path, mock_load_yaml, mock_apply_env, mock_load_dotenv, mock_config_data, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            ...
            with pytest.raises(RuntimeError, match="POSTGRES_PASSWORD is required but not found"):
                ConfigLoader()
    ...
    @patch("src.config_loader.ConfigLoader._load_yaml_config") 
    @patch("src.config_loader.Path")
    def test_empty_password_validation_security(self, mock_path, mock_load_yaml, mock_apply_env, mock_load_dotenv, mock_config_data):
        ...
        for empty_password in test_cases:
            with patch.dict(os.environ, {"POSTGRES_PASSWORD": empty_password}, clear=True):
                # Act & Assert
                ...
                assert "Please set the POSTGRES_PASSWORD environment variable" in str(exc_info.value)
    ...
    @patch("src.config_loader.ConfigLoader._apply_environment_overrides")
    @patch("src.config_loader.ConfigLoader._load_yaml_config")
    def test_environment_variable_type_conversion_security(self, mock_load_yaml, mock_apply_env, mock_load_dotenv, mock_config_data):
        ...
        with patch.dict(os.environ, {"POSTGRES_PASSWORD": "test_password", **test_cases}, clear=True):
            # Act
            config_loader = ConfigLoader()
    ...
    @patch("src.config_loader.ConfigLoader._apply_environment_overrides")
    @patch("src.config_loader.ConfigLoader._load_yaml_config")
    def test_config_loader_edge_case_security(self, mock_load_yaml, mock_apply_env, mock_load_dotenv, mock_config_data):
    ...
    @patch("src.config_loader.ConfigLoader._apply_environment_overrides")
    @patch("src.config_loader.ConfigLoader._load_yaml_config")
    def test_log_level_validation_prevents_injection(self, mock_load_yaml, mock_apply_env, mock_load_dotenv, mock_config_data, caplog):
