...
class TestStreamSearchIssues:
    ...
    def test_stream_search_issues_max_pages(self, mock_jira_client) -> None:
        ...
        issues = list(
            client.stream_search_issues("project = TEST", page_size=1, max_pages=3),
        )
...
class TestCachedOperations:
    ...
    def test_cached_statuses_get(self) -> None:
        ...
        mock_response = Mock()
        mock_response.json.return_value = {"_embedded": {"elements": mock_statuses}}
        mock_response.raise_for_status.return_value = None
...
class TestMemoryEfficiency:
    ...
    def test_streaming_memory_usage(self, mock_jira_client) -> None:
        ...
        issue_count = 0
        for _issue in client.stream_search_issues("project = TEST", page_size=50):
            issue_count += 1
