...
class TestEndToEndUserAssociationMigration:
    ...
    def test_end_to_end_fresh_mapping_workflow(self, seeded_migrator, metrics_collector, test_jira_issue, caplog):
        ...
        assert work_package_data["author_id"] == 456  # jane.smith's ID (will be refreshed)
    ...
    def test_end_to_end_stale_refresh_success_workflow(self, seeded_migrator, metrics_collector, mock_jira_client, caplog):
        ...
        with patch.object(seeded_migrator, '_save_enhanced_mappings'):
            with patch.object(seeded_migrator, '_validate_refreshed_user', return_value={"is_valid": True}):
                with caplog.at_level(logging.DEBUG):
                    # Execute staleness check with auto-refresh
                    mapping = seeded_migrator.get_mapping_with_staleness_check('jane.smith', auto_refresh=True)
        ...
        assert metrics_collector.get_tagged_counter(
            ...
            {'success': 'true', 'username': 'jane.smith', 'trigger': 'auto_refresh'}
        ) == 1
        ...
        assert any('Successfully refreshed mapping for jane.smith' in log for log in debug_logs)

    def test_end_to_end_stale_refresh_failure_workflow(self, seeded_migrator, metrics_collector, mock_jira_client, caplog):
        ...
        with patch.object(seeded_migrator, '_save_enhanced_mappings'):
            with caplog.at_level(logging.DEBUG):
                # Execute staleness check with auto-refresh
                mapping = seeded_migrator.get_mapping_with_staleness_check('bob.wilson', auto_refresh=True)
        ...
        assert metrics_collector.get_tagged_counter(
            'staleness_refreshed_total',
            {'success': 'false', 'username': 'bob.wilson', 'trigger': 'auto_refresh'}
        ) == 1
        
        # Verify logging for failed workflow
        debug_logs = [record.message for record in caplog.records if record.levelname == 'DEBUG']
        assert any('Failed to refresh mapping for bob.wilson' in log for log in debug_logs)

    def test_end_to_end_cache_miss_workflow(self, seeded_migrator, metrics_collector, test_jira_issue, caplog):
    ...
    def test_end_to_end_fallback_skip_workflow(self, seeded_migrator, metrics_collector, mock_jira_client, caplog):
        ...
        with patch.object(seeded_migrator, '_save_enhanced_mappings'):
            with patch.object(seeded_migrator, '_validate_refreshed_user', return_value={
                "is_valid": False, 
        ...
        assert any('Skipping user mapping for bob.wilson' in log for log in warning_logs)

    def test_end_to_end_fallback_create_placeholder_workflow(self, mock_jira_client, mock_op_client, metrics_collector, caplog):
    ...
    def test_end_to_end_batch_refresh_workflow(self, seeded_migrator, metrics_collector, mock_jira_client, caplog):
        ...
        with patch.object(seeded_migrator, '_save_enhanced_mappings'):
            with patch.object(seeded_migrator, '_validate_refreshed_user', return_value={"is_valid": True}):
                with caplog.at_level(logging.DEBUG):
                    ...
                    results = seeded_migrator.batch_refresh_stale_mappings(['jane.smith', 'bob.wilson'])
        ...
        success_count = metrics_collector.get_tagged_counter(
            'staleness_refreshed_total',
            {'success': 'true', 'username': 'jane.smith', 'trigger': 'batch_refresh', 'attempts': '1'}
        )
        assert success_count == 1
        
        failure_count = metrics_collector.get_tagged_counter(
            'staleness_refreshed_total', 
            {'success': 'false', 'username': 'bob.wilson', 'trigger': 'batch_refresh', 'attempts': '1'}
        )
        assert failure_count == 1

    def test_end_to_end_migration_workflow_with_all_scenarios(self, seeded_migrator, metrics_collector, mock_jira_client, test_jira_issue, caplog):
        ...
        def mock_jira_response(url):
            ...
            elif 'bob.wilson' in url:  # Stale user - refresh failure
                response.status_code = 404
            elif 'alice.brown' in url:  # Missing user - not found
                response.status_code = 404
            else:
                response.status_code = 404
            return response
        ...
        assert any('Staleness detected for jane.smith' in log for log in debug_logs)  # Stale
        assert any('Cache miss for' in log for log in debug_logs)  # Missing

    def test_end_to_end_metrics_persistence_and_aggregation(self, seeded_migrator, metrics_collector):
        ...
        seeded_migrator.check_and_handle_staleness('jane.smith', raise_on_stale=False)  # Stale
        ...
        missing_total = (
            metrics_collector.get_tagged_counter('staleness_detected_total', {'reason': 'missing', 'username': 'missing1'}) +
            metrics_collector.get_tagged_counter('staleness_detected_total', {'reason': 'missing', 'username': 'missing2'})
        )
        ...
        assert len(summary['tagged_counters']['staleness_detected_total']) == 3  # 3 unique tag combinations

    def test_end_to_end_error_recovery_and_resilience(self, seeded_migrator, metrics_collector, mock_jira_client, caplog):
        ...
        try:
            ...
            error_logs = [record.message for record in caplog.records if record.levelname in ['ERROR', 'WARNING']]
        ...
        mapping = seeded_migrator.check_and_handle_staleness('john.doe', raise_on_stale=False)
        assert mapping is not None  # Fresh mapping should work
        assert metrics_collector.get_counter('staleness_detected_total') == 0  # No staleness for fresh user 