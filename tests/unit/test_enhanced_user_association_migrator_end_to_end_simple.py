...
class TestSimplifiedEndToEndIntegration:
    ...
    def test_end_to_end_auto_refresh_success_workflow(
        self,
        migrator_with_seeded_cache,
    ) -> None:
        ...
        with (
            patch.object(migrator, "is_mapping_stale", return_value=True),
            ...
            result = migrator.get_mapping_with_staleness_check(
                "test.user",
                auto_refresh=True,
            )
            ...
            assert (
                result["openproject_username"] == "test.user"
            )  # Username, not firstname
            assert (
                result["openproject_email"] == "test@example.com"
            )  # Consistent with cache

            # Verify save was called (indicating refresh occurred)
            mock_save.assert_called()

    def test_end_to_end_staleness_detection_no_refresh(
        self,
        migrator_with_seeded_cache,
    ) -> None:
        ...
        with patch.object(migrator, "is_mapping_stale", return_value=True):
            result = migrator.check_and_handle_staleness(
                "test.user",
    ...
    def test_end_to_end_error_resilience_and_recovery(
        self,
        migrator_with_seeded_cache,
    ) -> None:
        ...
        migrator.op_client.get_user.return_value = mock_op_data
        migrator.op_client.get_user_by_email.return_value = (
            mock_op_data  # Add email lookup
        )
    ...
    def test_end_to_end_cache_management_workflow(
        self,
        migrator_with_seeded_cache,
    ) -> None:
    ...
    def test_end_to_end_metrics_integration(self, migrator_with_seeded_cache) -> None:
        ...
        with patch.object(migrator, "_save_enhanced_mappings"):
            initial_metrics = (
                migrator.metrics_collector.get_metrics()
            )  # Use get_metrics() method
