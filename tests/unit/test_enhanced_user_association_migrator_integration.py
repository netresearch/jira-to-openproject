...
class TestSecurityURLEncoding:
    ...
    @pytest.mark.parametrize("username, expected_encoded", [
        ...
        ("'; DROP TABLE users; --", "%27%3B%20DROP%20TABLE%20users%3B%20--"),  # SQL injection
        ("<script>alert('xss')</script>", "%3Cscript%3Ealert%28%27xss%27%29%3C%2Fscript%3E"),  # XSS
    ])
    def test_get_jira_user_info_url_encodes_special_characters(self, migrator_instance, username, expected_encoded):
        """
    ...
    def test_get_jira_user_info_handles_edge_case_usernames(self, migrator_instance, malicious_username):
        """
...
class TestAutoRefreshWorkflow:
    ...
    def test_get_jira_user_info_calls_refresh_for_stale_mapping(self, migrator_instance):
        """
        ...
        mock_user_response.json.return_value = [{"accountId": "123", "displayName": "Stale User"}]
        migrator_instance.jira_client.get.return_value = mock_user_response

        with patch.object(migrator_instance, 'is_mapping_stale', return_value=True) as mock_is_stale, \
    ...
    def test_get_jira_user_info_does_not_call_refresh_for_fresh_mapping(self, migrator_instance):
        """
        ...
        mock_user_response = MagicMock()
        mock_user_response.status_code = 200
        mock_user_response.json.return_value = [{"accountId": "456", "displayName": "Fresh User"}]
        migrator_instance.jira_client.get.return_value = mock_user_response

        with patch.object(migrator_instance, 'is_mapping_stale', return_value=False) as mock_is_stale, \
             patch.object(migrator_instance, 'refresh_user_mapping') as mock_refresh:
    ...
    def test_get_jira_user_info_caches_staleness_check_result(self, migrator_instance):
        """
...
class TestErrorHandlingAndResilience:
    ...
    def test_get_jira_user_info_handles_request_exception(self, migrator_instance, exception):
        """
    ...
    def test_get_jira_user_info_handles_malformed_json_response(self, migrator_instance):
        """
    ...
    def test_get_jira_user_info_handles_empty_user_list_response(self, migrator_instance):
        """
    ...
    def test_get_jira_user_info_handles_http_error_codes(self, migrator_instance, status_code):
        """
...
class TestRefreshUserMapping:
    ...
    def test_refresh_user_mapping_updates_existing_entry(self, migrator_instance):
        """
        ...
        fresh_jira_data = {"accountId": "xyz-789", "displayName": "New Name", "active": True}
        
        with patch.object(migrator_instance, '_get_jira_user_info', return_value=fresh_jira_data), \
             patch.object(migrator_instance, '_get_current_timestamp', return_value=new_timestamp):
    ...
    def test_refresh_user_mapping_creates_new_entry_when_none_exists(self, migrator_instance):
        """
        ...
        fresh_jira_data = {"accountId": "new-123", "displayName": "New User", "active": True}
        
        with patch.object(migrator_instance, '_get_jira_user_info', return_value=fresh_jira_data), \
             patch.object(migrator_instance, '_get_current_timestamp', return_value=timestamp), \
             patch.object(migrator_instance, '_get_openproject_user_info', return_value=None):
    ...
    def test_refresh_user_mapping_fails_gracefully_on_api_error(self, migrator_instance):
        """
        ...
        with patch.object(migrator_instance, '_get_jira_user_info', return_value=None):
            ...
            migrator_instance.logger.warning.assert_called_once_with(
                "Could not fetch fresh user info for %s", username
            )
    ...
    def test_refresh_user_mapping_handles_specific_exceptions(self, migrator_instance, exception):
        """
        Verify that refresh_user_mapping handles specific exception types correctly.
        """
        # Arrange
        username = "exception_user"
        with patch.object(migrator_instance, '_get_jira_user_info', side_effect=exception):
    ...
    def test_refresh_user_mapping_handles_unexpected_exceptions(self, migrator_instance):
        """
        ...
        with patch.object(migrator_instance, '_get_jira_user_info', side_effect=unexpected_error):
...
class TestConfigurationValidation:
    ...
    def test_load_staleness_config_with_valid_configuration(self, migrator_instance):
        """
    ...
    def test_load_staleness_config_with_invalid_duration_falls_back_to_defaults(self, mock_clients):
        """
    ...
    def test_load_staleness_config_with_missing_admin_user_id_warns(self, mock_clients):
        """
        ...
        with patch('src.utils.enhanced_user_association_migrator.config') as mock_config:
            ...
            warning_calls = [call for call in migrator.logger.warning.call_args_list 
...
class TestEdgeCasesAndBoundaries:
    ...
    def test_staleness_threshold_boundary_conditions(self, migrator_instance):
        """
    ...
    def test_malformed_timestamp_handling(self, migrator_instance):
        """
    ...
    def test_missing_lastRefreshed_field(self, migrator_instance):
        """
