...
class TestEnhancedUserAssociationMigratorMonitoring:
    ...
    def test_no_metrics_collector_graceful_handling(self, migrator_no_metrics) -> None:
        ...
        result = migrator_no_metrics.check_and_handle_staleness(
            "test.user",
            raise_on_stale=False,
        )
        assert result is None

    def test_cache_miss_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
        caplog,
        ...
        with caplog.at_level(logging.DEBUG):
            result = migrator_with_metrics.check_and_handle_staleness(
                "missing.user",
                raise_on_stale=False,
            )
    ...
    def test_staleness_detection_expired_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
    ) -> None:
        ...
        old_time = datetime.now(tz=UTC) - timedelta(
            hours=2,
    ...
    def test_staleness_detection_no_timestamp_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
    ) -> None:
        ...
        migrator_with_metrics.enhanced_user_mappings["no.timestamp"] = (
            no_timestamp_mapping
        )
    ...
    def test_staleness_detection_invalid_timestamp_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
    ) -> None:
        ...
        migrator_with_metrics.enhanced_user_mappings["invalid.timestamp"] = (
            invalid_timestamp_mapping
        )
    ...
    def test_refresh_success_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
        caplog,
        ...
        assert any(
            "Successfully refreshed mapping for test.user" in log for log in debug_logs
        )
    ...
    def test_refresh_failure_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
        caplog,
        ...
        assert any(
            "Failed to refresh mapping for test.user" in log for log in debug_logs
        )
    ...
    def test_batch_refresh_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
    ) -> None:
        ...
        with patch.object(
            ...
            migrator_with_metrics.batch_refresh_stale_mappings(
                ["user1", "user2"],
            )
    ...
    def test_fallback_skip_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
    ) -> None:
        """Test monitoring of skip fallback strategy execution."""
        current_mapping = {"jira_username": "test.user", "metadata": {}}

        with patch.object(migrator_with_metrics, "_save_enhanced_mappings"):
            migrator_with_metrics._execute_skip_fallback(
                "test.user",
                "user_not_found",
                current_mapping,
            )
    ...
    def test_fallback_create_placeholder_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
    ) -> None:
    ...
    def test_bulk_staleness_detection_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
    ) -> None:
    ...
    def test_thread_safety_metrics_collection(
        self,
        migrator_with_metrics,
        metrics_collector,
    ) -> None:
        ...
        def concurrent_staleness_check(username_prefix) -> None:
            """Simulate concurrent staleness checks."""
            for i in range(10):
                username = f"{username_prefix}.user{i}"
                migrator_with_metrics.check_and_handle_staleness(
                    username,
                    raise_on_stale=False,
                )
        ...
        for thread_id in range(5):
            thread = threading.Thread(
                target=concurrent_staleness_check,
                args=(f"thread{thread_id}",),
            )
    ...
    def test_monitoring_error_resilience(self, migrator_with_metrics) -> None:
    ...
    def test_comprehensive_workflow_monitoring(
        self,
        migrator_with_metrics,
        metrics_collector,
        caplog,
        ...
        with (
            ...
            patch.object(migrator_with_metrics, "_save_enhanced_mappings"),
    ...
    def test_metrics_summary_functionality(
        ...
        with patch.object(
            ...
            migrator_with_metrics.get_mapping_with_staleness_check(
                "user1",
                auto_refresh=True,
            )
        ...
        assert (
            summary["total_metrics"] >= 3
        )  # At least staleness_detected, staleness_refreshed, mapping_fallback
        assert summary["total_count"] >= 4  # At least 4 events tracked

    def test_monitoring_with_no_metrics_collector(
        self,
        migrator_no_metrics,
        caplog,
    ) -> None:
