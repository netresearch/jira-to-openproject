...
class TestIdempotencyDecorators:
    ...
    def test_batch_idempotent_decorator_cache_miss(self):
        ...
        result1 = test_function(["item1", "item2"], headers={"X-Idempotency-Key": valid_uuid})
        # The batch processor transforms the result
        assert result1["data"]["processed"] == ["item1", "item2"]
        assert result1["data"]["count"] == 1
        assert result1["success"] == True
        assert result1["idempotent"] == True
        assert call_count == 1

    def test_batch_idempotent_decorator_cache_hit(self):
    ...
    def test_api_idempotent_flask_headers(self):
    ...
    def test_api_idempotent_django_headers(self):
    ...
    def test_simple_idempotent_no_headers(self):
    ...
    def test_custom_header_extractor(self):
    ...
    def test_batch_result_processor(self):
    ...
    def test_batch_result_processor_with_errors(self):
    ...
    def test_result_processor_error_handling(self):
    ...
    def test_header_extractor_error_handling(self):
        """Test error handling in header extractors."""
        def failing_extractor(*args, **kwargs):
            raise KeyError("Header extraction failed")
    ...
    def test_extract_headers_from_kwargs_default(self):
    ...
    def test_extract_headers_from_kwargs_custom_key(self):
    ...
    def test_extract_headers_from_kwargs_missing(self):
    ...
    def test_extract_headers_from_request_no_request(self):
    ...
    def test_extract_headers_from_request_flask_style(self):
        """Test extract_headers_from_request with Flask request."""
        extractor = extract_headers_from_request()
        
        mock_request = Mock()
        mock_request.headers = {"X-Idempotency-Key": "flask", "Content-Type": "application/json"}
    ...
    def test_extract_headers_from_request_django_style(self):
    ...
    def test_concurrent_decorator_access(self):
        ...
        assert call_counts["count"] == 1, f"Function should only be called once, but was called {call_counts['count']} times"
        
        # All should get the same result due to idempotency
        unique_results = set(json.dumps(result, sort_keys=True) for result in results.values())
        assert len(unique_results) == 1, "All concurrent calls should return same result"

    def test_decorator_with_no_idempotency_manager(self):
        """Test decorator behavior when idempotency manager fails to initialize."""
        with patch('src.utils.idempotency_decorators.get_idempotency_manager') as mock_get_manager:
    ...
    def test_result_serialization_in_decorator(self):
