...
class TestMetricsCollector:
    ...
    def test_increment_counter_different_tags(self) -> None:
    ...
    def test_get_metrics_comprehensive(self) -> None:
    ...
    def test_thread_safety(self) -> None:
        ...
        assert (
            collector.get_tagged_counter("tagged_thread_metric", {"thread": "worker"})
            == 500
        )
    ...
    def test_staleness_metrics_integration(self) -> None:
        ...
        assert (
            collector.get_tagged_counter(
                "staleness_detected_total",
                {"reason": "missing", "username": "user1"},
            )
            == 1
        )
        assert (
            collector.get_tagged_counter(
                "mapping_fallback_total",
                {"fallback_strategy": "skip", "reason": "validation_failed"},
            )
            == 1
        )
