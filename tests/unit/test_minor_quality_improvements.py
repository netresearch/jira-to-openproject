...
class TestConfigLoaderSecurityLogging:
    ...
    def test_exception_logging_does_not_expose_sensitive_path_info(
        self,
        temp_dir,
    ) -> None:
...
""",
        ...
        with (
            ...
            patch("src.config_loader.config_logger") as mock_logger,
        ):
            # Act
...
""",
        ...
        with patch.dict("os.environ", {"POSTGRES_PASSWORD_FILE": str(secret_file)}):
...
class TestDurationParsingWhitespaceHandling:
    ...
    @pytest.mark.parametrize(
        ("duration_with_whitespace", "expected_seconds"),
        [
    ...
    def test_parse_duration_handles_whitespace_defensively(
        self,
        migrator_instance,
        duration_with_whitespace,
        expected_seconds,
    ) -> None:
    ...
    @pytest.mark.parametrize(
        "invalid_duration_with_whitespace",
        [
            " ",  # Only whitespace
    ...
    def test_parse_duration_rejects_whitespace_only_or_invalid_with_whitespace(
        self,
        migrator_instance,
        invalid_duration_with_whitespace,
    ) -> None:
    ...
    def test_parse_duration_empty_string_after_strip_raises_error(
        self,
        migrator_instance,
    ) -> None:
    ...
    def test_parse_duration_preserves_original_error_messages_for_invalid_formats(
        self,
        migrator_instance,
    ) -> None:
        ...
        with pytest.raises(ValueError, match="Invalid duration format"):
            migrator_instance._parse_duration(
                "  1x  ",
            )  # Invalid unit, even after strip


class TestQualityImprovementIntegration:
    """Integration tests for quality improvements."""

    def test_config_loader_and_migrator_quality_improvements_work_together(
        self,
        temp_dir,
        mock_clients,
    ) -> None:
