...
class TestPaginationImplementation:
    ...
    def test_iter_project_issues_basic_functionality(self):
    ...
    def test_iter_project_issues_handles_retry_logic(self):
    ...
    def test_iter_project_issues_project_not_found(self):
        """Test that project not found errors are handled gracefully."""
        # Mock the project check to fail
        self.migration.jira_client.jira.project.side_effect = Exception("Project not found")
    ...
    def test_extract_jira_issues_uses_generator(self):
        ...
        with patch.object(self.migration, 'iter_project_issues') as mock_iter, \
             patch.object(self.migration, '_save_to_json') as mock_save:
    ...
    def test_get_current_entities_for_type_uses_generator(self):
        ...
        with patch('src.migrations.work_package_migration.config') as mock_config:
            ...
            with patch.object(self.migration, 'iter_project_issues') as mock_iter:
                ...
                assert len(result) >= 2  # Should return processed issues from both projects
    ...
    def test_memory_efficiency_no_accumulation(self):
        ...
        with patch('src.migrations.work_package_migration.config') as mock_config:
            ...
            with patch.object(self.migration, '_fetch_issues_with_retry') as mock_fetch:
                ...
                batch_responses.append([create_mock_issue(i, "TEST") for i in range(1, 51)])
                
                # Second batch (50 issues - full batch, continue)
                batch_responses.append([create_mock_issue(i, "TEST") for i in range(51, 101)])
                
                # Third batch (50 issues - full batch, continue)
                batch_responses.append([create_mock_issue(i, "TEST") for i in range(101, 151)])
