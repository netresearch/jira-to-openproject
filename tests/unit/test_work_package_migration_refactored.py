...
class TestWorkPackageMigrationRefactored(unittest.TestCase):
    ...
    def test_execute_time_entry_migration_successful(self) -> None:
        ...
        with patch.object(self.migration, '_perform_time_entry_migration', return_value=expected_result):
            # Execute
            result = self.migration._execute_time_entry_migration()
            
            # Verify
            self.assertEqual(result, expected_result)
            self.assertEqual(result["total_time_entries"]["migrated"], 73)

    def test_execute_time_entry_migration_raises_migration_error_on_failure(self) -> None:
        """Test that MigrationError is raised when time entry migration fails."""
        # Setup - Mock internal migration to raise an exception
        with patch.object(self.migration, '_perform_time_entry_migration', side_effect=Exception("Database connection lost")):
            # Execute & Verify
            with pytest.raises(MigrationError, match="Failed to execute time entry migration.*Database connection lost"):
    ...
    def test_execute_time_entry_migration_handles_nested_exceptions(self) -> None:
        ...
        with patch.object(self.migration, '_perform_time_entry_migration', side_effect=original_exception):
            # Execute & Verify
            ...
            self.assertIn("Invalid work package ID format", str(exc_info.value.__cause__))
    ...
    def test_work_package_creation_ruby_script_exception_handling(self) -> None:
        ...
        self.mock_op_client.execute_query.side_effect = QueryExecutionError("Rails validation error: Name cannot be blank")
        
        # Mock the migration method that uses the Ruby script
        with patch.object(self.migration, '_create_work_packages_for_project') as mock_create:
            mock_create.side_effect = QueryExecutionError("Rails validation error: Name cannot be blank")
    ...
    def test_work_package_creation_continues_on_individual_failures(self) -> None:
        ...
        def mock_process_project(project_key):
            if project_key == "PROJECT-2":
                raise QueryExecutionError(f"Rails error for {project_key}")
            return {"success": True, "project": project_key, "work_packages_created": 10}
        ...
        # Verify
        self.assertEqual(len(processed_projects), 2)  # PROJECT-1 and PROJECT-3 succeeded
        self.assertEqual(len(failed_projects), 1)     # PROJECT-2 failed
        self.assertEqual(failed_projects[0]["project_key"], "PROJECT-2")
        self.assertIn("Rails error for PROJECT-2", failed_projects[0]["error"])
    ...
    def test_ruby_script_no_longer_contains_error_dictionaries(self) -> None:
        ...
        # Setup
        self.mock_op_client.execute_query.return_value = "Work packages created successfully"
        
        # Execute - trigger Ruby script generation
        with patch.object(self.migration, '_generate_work_package_ruby_script') as mock_script:
            ...
            self.assertNotIn("rescue => e", script)
            self.assertNotIn("error:", script)
            self.assertNotIn("status: 'error'", script)
            self.assertNotIn("message: e.message", script)
            self.assertNotIn("backtrace:", script)
            
            # Verify it has proper success patterns
            self.assertIn("WorkPackage.create!", script)
            self.assertIn("result.to_json", script)
    ...
    def test_calling_code_handles_new_exception_patterns(self) -> None:
        ...
        with patch.object(self.migration, '_execute_time_entry_migration', side_effect=MigrationError("Time entry migration failed")):
        ...
        # Verify
        self.assertEqual(migration_result["time_entry_migration"]["status"], "failed")
        self.assertIn("Time entry migration failed", migration_result["time_entry_migration"]["error"])
        self.assertEqual(len(migration_result["errors"]), 1)
    ...
    def test_time_entry_migration_with_empty_work_packages(self) -> None:
        ...
        with patch.object(self.migration, '_get_migrated_work_packages', return_value=[]):
            # This should not raise an exception, but return empty results
            with patch.object(self.migration, '_perform_time_entry_migration') as mock_perform:
                ...
                # Verify
                self.assertEqual(result["total_time_entries"]["migrated"], 0)
                self.assertEqual(result["total_time_entries"]["failed"], 0)
    ...
    def test_migration_error_preserves_original_context(self) -> None:
        ...
        with patch.object(self.migration, '_perform_time_entry_migration', side_effect=original_error):
            # Execute & Verify
            with pytest.raises(MigrationError) as exc_info:
                self.migration._execute_time_entry_migration()
            ...
            self.assertIn("Failed to execute time entry migration", str(migration_error))
            self.assertIsInstance(migration_error.__cause__, ConnectionError)
            self.assertIn("Network connection lost", str(migration_error.__cause__))
    ...
    def test_batch_processing_error_handling(self) -> None:
        ...
        def process_batch(batch):
            if batch["batch_id"] == 2:
                raise QueryExecutionError(f"Batch {batch['batch_id']} failed due to Rails error")
            return {"success": True, "batch_id": batch["batch_id"], "processed_count": len(batch["projects"])}
        ...
        # Verify
        self.assertEqual(len(successful_batches), 2)  # Batch 1 and 3 succeeded
        self.assertEqual(len(failed_batches), 1)      # Batch 2 failed
        self.assertEqual(failed_batches[0]["batch_id"], 2)
        self.assertEqual(successful_batches[0]["batch_id"], 1)
        self.assertEqual(successful_batches[1]["batch_id"], 3) 